...
function mount$1(glide, extensions, events) {
  var components = {};
...
function mergeOptions$1(defaults, settings) {
  var options = _extends$1({}, defaults, settings); // `Object.assign` do not deeply merge objects, so we
  // have to do it manually for every nested object
  // in options. Although it does not look smart,
  // it's smaller and faster than some fancy
  // merging deep-merge algorithm script.
...
var Glide$2 = function () {
  ...
  function Glide(selector) {
    ...
    this._c = {};
    this._t = [];
    this._e = new EventsBus$1();
    this.disabled = false;
    this.selector = selector;
    this.settings = mergeOptions$1(defaults$1, options);
    this.index = this.settings.startAt;
  }
  /**
   * Initializes glide.
  ...
  createClass$1(Glide, [{
    ...
    value: function mount$$1() {
      ...
      if (isObject$1(extensions)) {
        this._c = mount$1(this, extensions, this._e);
      } else {
        warn$1('You need to provide a object on `mount()`');
      }
    ...
    value: function mutate() {
      ...
      if (isArray$1(transformers)) {
        ...
        warn$1('You need to provide a array on `mutate()`');
      }
    ...
    value: function update() {
      var settings = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
      this.settings = mergeOptions$1(this.settings, settings);
...
function Run$1(Glide, Components, Events) {
  var Run = {
    /**
     * Initializes autorunning of the glide.
     *
    ...
    calculate: function calculate() {
      ...
      var steps = move.steps,
          direction = move.direction;
      var countableSteps = isNumber$1(toInt$1(steps)) && toInt$1(steps) !== 0;
      ...
      switch (direction) {
        case '>':
          if (steps === '>') {
            ...
            Glide.index += Math.min(length - Glide.index, -toInt$1(steps));
          } else {
            Glide.index++;
          }
        ...
        case '<':
          if (steps === '<') {
            ...
            Glide.index -= Math.min(Glide.index, toInt$1(steps));
          } else {
            Glide.index--;
          }
        ...
        default:
          warn$1('Invalid direction pattern [' + direction + steps + '] has been used');
          break;
      }
    },
  ...
  define$1(Run, 'move', {
    ...
    set: function set(value) {
      ...
      this._m = {
        direction: value.substr(0, 1),
        steps: step ? toInt$1(step) ? toInt$1(step) : step : 0
      };
    }
  });
  define$1(Run, 'length', {
    ...
    get: function get() {
      ...
      if (Glide.isType('slider') && settings.focusAt !== 'center' && settings.bound) {
        return length - 1 - (toInt$1(settings.perView) - 1) + toInt$1(settings.focusAt);
      }
...
function Gaps$1(Glide, Components, Events) {
  var Gaps = {
    /**
     * Applies gaps between slides. First and last
     * slides do not receive it's edge margins.
    ...
    apply: function apply(slides) {
      for (var i = 0, len = slides.length; i < len; i++) {
        ...
        if (i !== 0) {
          style[MARGIN_TYPE$1[direction][0]] = this.value / 2 + 'px';
        } else {
          style[MARGIN_TYPE$1[direction][0]] = '';
        }

        if (i !== slides.length - 1) {
          style[MARGIN_TYPE$1[direction][1]] = this.value / 2 + 'px';
        } else {
          style[MARGIN_TYPE$1[direction][1]] = '';
        }
      }
    },
  ...
  define$1(Gaps, 'value', {
     ...
     */
    get: function get() {
      return toInt$1(Glide.settings.gap);
    }
  });
  define$1(Gaps, 'grow', {
    /**
  ...
  Events.on(['build.after', 'update'], throttle$1(function () {
    Gaps.apply(Components.Html.wrapper.children);
  }, 30));
  /**
   * Remove gaps:
...
var TRACK_SELECTOR$1 = '[data-glide-el="track"]';

function Html$1(Glide, Components) {
  var Html = {
    /**
     * Setup slider HTML nodes.
     *
    ...
    mount: function mount() {
      this.root = Glide.selector;
      this.track = this.root.querySelector(TRACK_SELECTOR$1);
      this.slides = Array.prototype.slice.call(this.wrapper.children).filter(function (slide) {
        return !slide.classList.contains(Glide.settings.classes.cloneSlide);
      });
    }
  ...
  define$1(Html, 'track', {
    ...
    set: function set(t) {
      if (exist$1(t)) {
        ...
        warn$1('Could not find track element. Please use ' + TRACK_SELECTOR$1 + ' attribute.');
      }
    }
  });
...
function Peek$1(Glide, Components, Events) {
  var Peek = {
    /**
     * Setups how much to peek based on settings.
     *
  ...
  define$1(Peek, 'value', {
    ...
    set: function set(value) {
      if (isObject$1(value)) {
        value.before = toInt$1(value.before);
        value.after = toInt$1(value.after);
      } else {
        value = toInt$1(value);
      }
...
function Move$1(Glide, Components, Events) {
  var Move = {
    /**
     * Constructs move component.
     *
  ...
  define$1(Move, 'offset', {
     ...
     */
    set: function set(value) {
      Move._o = !isUndefined$1(value) ? toInt$1(value) : 0;
    }
  });
...
function Sizes$1(Glide, Components, Events) {
  var Sizes = {
    /**
     * Setups dimentions of slides.
     *
...
function Build$1(Glide, Components, Events) {
  var Build = {
    /**
     * Init glide building. Adds classes, sets
     * dimensions and setups initial state.
...
function Clones$1(Glide, Components, Events) {
  var Clones = {
    /**
     * Create pattern map and collect slides to be cloned.
     */
...
function Resize$1(Glide, Components, Events) {
  /**
   * Instance of the binder for DOM Events.
   *
   * @type {EventsBinder}
  ...
  var Resize = {
     ...
     */
    bind: function bind() {
      Binder.on('resize', window, throttle$1(function () {
        Events.emit('resize');
      }, Glide.settings.throttle));
    },
...
var VALID_DIRECTIONS$1 = ['ltr', 'rtl'];
var FLIPED_MOVEMENTS$1 = {
  '>': '<',
  '<': '>',
  '=': '='
};

function Direction$1(Glide, Components, Events) {
  var Direction = {
    /**
     * Setups gap value based on settings.
     *
    ...
    resolve: function resolve(pattern) {
      ...
      if (this.is('rtl')) {
        return pattern.split(token).join(FLIPED_MOVEMENTS$1[token]);
      }
  ...
  define$1(Direction, 'value', {
     ...
     */
    set: function set(value) {
      if (VALID_DIRECTIONS$1.indexOf(value) > -1) {
        Direction._v = value;
      } else {
        warn$1('Direction value must be `ltr` or `rtl`');
      }
    }
  });
  /**
...
function Rtl$1(Glide, Components) {
  return {
    /**
     * Negates the passed translate if glide is in RTL option.
     *
...
function Grow$1(Glide, Components) {
  return {
    /**
     * Adds to the passed translate width of the half of clones.
     *
...
function Peeking$1(Glide, Components) {
  return {
    /**
     * Modifies passed translate value with a `peek` setting.
     *
...
function Focusing$1(Glide, Components) {
  return {
    /**
     * Modifies passed translate value with index in the `focusAt` setting.
     *
...
function mutator$1(Glide, Components, Events) {
  /**
   * Merge instance transformers with collection of default transformers.
   * It's important that the Rtl component be last on the list,
   * so it reflects all previous transformations.
   ...
   * @type {Array}
   */
  var TRANSFORMERS = [Gap$1, Grow$1, Peeking$1, Focusing$1].concat(Glide._t, [Rtl$1]);
  return {
    /**
     * Piplines translate value with registered transformers.
     *
    ...
    mutate: function mutate(translate) {
      for (var i = 0; i < TRANSFORMERS.length; i++) {
        ...
        if (isFunction$1(transformer) && isFunction$1(transformer().modify)) {
          ...
          warn$1('Transformer should be a function that returns an object with `modify()` method');
        }
      }
...
function Translate$1(Glide, Components, Events) {
  var Translate = {
    /**
     * Sets value of translate on HTML element.
     *
     ...
     */
    set: function set(value) {
      var transform = mutator$1(Glide, Components).mutate(value);
      Components.Html.wrapper.style.transform = 'translate3d(' + -1 * transform + 'px, 0px, 0px)';
    },
...
function Transition$1(Glide, Components, Events) {
  /**
   * Holds inactivity status of transition.
   * When true transition is not applied.
   *
...
function sortBreakpoints$1(points) {
  if (isObject$1(points)) {
    return sortKeys$1(points);
  } else {
    warn$1('Breakpoints option must be an object');
  }
...
function breakpoints(Glide, Components, Events) {
  ...
  var points = sortBreakpoints$1(settings.breakpoints);
  /**
   * Cache initial settings before overwritting.
   *
   * @type {Object}
  ...
  Binder.on('resize', window, throttle$1(function () {
    Glide.settings = mergeOptions$1(settings, Breakpoints.match(points));
  }, Glide.settings.throttle));
  /**
   * Resort and update default settings:
   * - on reinit via API, so breakpoint matching will be performed with options
  ...
  Events.on('update', function () {
    points = sortBreakpoints$1(points);
...
var Glide$1$1 = function (_Core) {
  ...
  createClass$1(Glide$$1, [{
    ...
    value: function mount() {
      var extensions = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
      return get$1(Glide$$1.prototype.__proto__ || Object.getPrototypeOf(Glide$$1.prototype), 'mount', this).call(this, _extends$1({}, COMPONENTS$1, extensions));
    }
  }]);
  return Glide$$1;
}(Glide$2);
