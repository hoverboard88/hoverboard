...
class RawDataParser
{
    ...
    protected $cfg = [
        ...
        'ignore_filter_decoding_errors' => \true,
    ...
    protected function decodeXrefStream($pdfData, $startxref, $xref = [])
    {
        ...
        $xrefcrs = $this->getIndirectObject($pdfData, $xref, $xrefobj[1], $startxref, \true);
        if (!isset($xref['trailer']) || empty($xref['trailer'])) {
            // get only the last updated version
            $xref['trailer'] = [];
            $filltrailer = \true;
    ...
    protected function getRawObject($pdfData, $offset = 0)
    {
        ...
        switch ($char) {
            ...
            case '/':
                ...
                $objtype = $char;
                ++$offset;
                $pregResult = \preg_match('/^([^\\x00\\x09\\x0a\\x0c\\x0d\\x20\\s\\x28\\x29\\x3c\\x3e\\x5b\\x5d\\x7b\\x7d\\x2f\\x25]+)/', \substr($pdfData, $offset, 256), $matches);
                if (1 == $pregResult) {
                    $objval = $matches[1];
                    // unescaped value
            ...
            default:
                if ('endobj' == \substr($pdfData, $offset, 6)) {
                    ...
                    if (1 == \preg_match('/^([\\r]?[\\n])/isU', \substr($pdfData, $offset), $matches)) {
                        $offset += \strlen($matches[0]);
                        $pregResult = \preg_match('/(endstream)[\\x09\\x0a\\x0c\\x0d\\x20]/isU', \substr($pdfData, $offset), $matches, \PREG_OFFSET_CAPTURE);
                        if (1 == $pregResult) {
    ...
    public function parseData($data)
    {
        ...
        foreach ($xref['xref'] as $obj => $offset) {
            if (!isset($objects[$obj]) && $offset > 0) {
                ...
                $objects[$obj] = $this->getIndirectObject($pdfData, $xref, $obj, $offset, \true);
            }
        }
        return [$xref, $objects];
    }
