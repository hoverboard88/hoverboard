...
use SearchWP\Dependencies\Smalot\PdfParser\XObject\Form;
use SearchWP\Dependencies\Smalot\PdfParser\XObject\Image;
/**
 * Class PDFObject
 */
class PDFObject
            ...
            foreach ($commands as $command) {
                switch ($command[self::OPERATOR]) {
                    ...
                    case 'Td':
                        ...
                        $y = \array_pop($args);
                        $x = \array_pop($args);
                        if ((float) $x <= 0 || \false !== $current_position_td['y'] && (float) $y < (float) $current_position_td['y']) {
                            // vertical offset
                            $text .= "\n";
    ...
    public function getCommandsText($text_part, &$offset = 0)
    {
        ...
        while ($offset < \strlen($text_part)) {
            ...
            switch ($char) {
                case '/':
                    $type = $char;
                    if (\preg_match('/^\\/([A-Z0-9\\._,\\+]+\\s+[0-9.\\-]+)\\s+([A-Z]+)\\s*/si', \substr($text_part, $offset), $matches)) {
                        $operator = $matches[2];
                        $command = $matches[1];
                        $offset += \strlen($matches[0]);
                    } elseif (\preg_match('/^\\/([A-Z0-9\\._,\\+]+)\\s+([A-Z]+)\\s*/si', \substr($text_part, $offset), $matches)) {
                        $operator = $matches[2];
                        $command = $matches[1];
                        $offset += \strlen($matches[0]);
                    }
                ...
                default:
                    if ('ET' == \substr($text_part, $offset, 2)) {
                        break;
                    } elseif (\preg_match('/^\\s*(?P<data>([0-9\\.\\-]+\\s*?)+)\\s+(?P<id>[A-Z]{1,3})\\s*/si', \substr($text_part, $offset), $matches)) {
                        $operator = \trim($matches['id']);
                        $command = \trim($matches['data']);
                        $offset += \strlen($matches[0]);
                    } elseif (\preg_match('/^\\s*([0-9\\.\\-]+\\s*?)+\\s*/si', \substr($text_part, $offset), $matches)) {
                        $type = 'n';
                        $command = \trim($matches[0]);
                        $offset += \strlen($matches[0]);
