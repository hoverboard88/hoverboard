...
use SearchWP\Dependencies\Smalot\PdfParser\Encoding\WinAnsiEncoding;
...
class Font extends PDFObject
{
     ...
     * @return string|bool
     */
    public function translateChar(string $char, bool $use_default = \true)
    {
        ...
        if ($this->has('ToUnicode')) {
            ...
            if (\preg_match_all('/begincodespacerange(?P<sections>.*?)endcodespacerange/s', $content, $matches)) {
                foreach ($matches['sections'] as $section) {
                    ...
                    $this->tableSizes = ['from' => \max(1, \strlen(\current($matches['from'])) / 2), 'to' => \max(1, \strlen(\current($matches['to'])) / 2)];
                    break;
                }
            }
    ...
    {
        $pdfToIconvEncodingNameMap = ['StandardEncoding' => 'ISO-8859-1', 'MacRomanEncoding' => 'MACINTOSH', 'WinAnsiEncoding' => 'CP1252'];
        return \array_key_exists($pdfEncodingName, $pdfToIconvEncodingNameMap) ? $pdfToIconvEncodingNameMap[$pdfEncodingName] : null;
    }
    /**
