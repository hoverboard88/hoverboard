<template>
	<div>
		...
		<searchwp-notice
			v-if="newDoSuggestions"
			:type="'warning'"
			:dismissable="true"
			v-on:dismissed="dismissed_do_suggestions_note = true">
			<component :is="translatedDoSuggestionsNote"></component>
		</searchwp-notice>
		<searchwp-notice
			v-if="newQuotedSearchSupport"
			:type="'warning'"
			:dismissable="true"
			v-on:dismissed="dismissed_quoted_search_support_note = true">
			<component :is="translatedSupportQuotedSearchesNote"></component>
		</searchwp-notice>
		<ul :class="[ 'searchwp-settings-advanced__settings', saving ? 'searchwp-settings--is-saving' : '' ]">
			<li>
				<setting
					...
					:value="debugging"
					:disabled="saving"
					v-on:change="updateSetting($event)"></setting>
			</li>
			<li><setting
			...
			<li><setting
					...
					:disabled="saving"
					v-on:change="updateSetting($event);flagIndex()"></setting>
			</li>
			<li><setting
			...
			<li class="searchwp-admin-search"><setting
					...
					:disabled="saving"
					v-on:change="updateSetting($event)"></setting>
				<v-popover offset="6" placement="bottom" ref="adminSearchPopover">
					...
					<template slot="popover">
						<ul class="searchwp-popover-actions">
							<li v-for="engine in engines" :key="engine.name">
								<button @click="setAdminEngine(engine)">{{ engine.label }}</button>
							</li>
						</ul>
					</template>
				</v-popover>
			...
			<li><setting
					:name="'do_suggestions'"
					:label="i18n.automaticDidYouMean"
					:value="do_suggestions"
					:disabled="saving"
					v-on:change="updateSetting($event)"></setting>
			</li>
			<li><setting
					:name="'quoted_search_support'"
					:label="i18n.supportQuotedSearches"
					:value="quoted_search_support"
					:disabled="saving"
					v-on:change="updateSetting($event)"></setting>
			</li>
			<li><setting
...
export default {
	...
	computed: {
		...
		translatedDoSuggestionsNote() {
			return {
				template: '<span>' + this.i18n.doSuggestionsNote + '</span>'
			}
		},
		translatedSupportQuotedSearchesNote() {
			return {
