...
class Synonyms {
	...
	public function apply( string $search_string, Query $query ) {
		if ( apply_filters( 'searchwp\synonyms\partial_matches', \SearchWP\Settings::get_single( 'partial_matches', 'boolean' ), [
			...
			foreach ( $sources as $source ) {
				...
				if ( 1 === preg_match( $pattern, $source, $match ) ) {
					if ( isset( $synonym['replace'] ) && ! empty( $synonym['replace'] ) ) {
						$search_string = preg_replace( $pattern, $synonym['synonyms'], $search_string );
					} else {
					...
					foreach ( $terms as $index => $term ) {
						...
						$term_pattern = "/$term/ui";
						if ( 1 === preg_match( $term_pattern, $synonym['sources'], $match ) ) {
							if ( isset( $synonym['replace'] ) && ! empty( $synonym['replace'] ) ) {
								$terms[ $index ] = preg_replace( $term_pattern, $synonym['synonyms'], $terms[ $index ] );
							} else {
								$terms[ $index ] .= ' ' . $synonym['synonyms'];
							}
						}
					}

					$search_string = implode( ' ', $terms );
				}
			}
		}
