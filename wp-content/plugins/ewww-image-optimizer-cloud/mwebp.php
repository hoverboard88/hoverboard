...
function ewww_image_optimizer_webp_migrate_preview() {
	...
	?>
	<div class="wrap">
	<h1><?php esc_html_e( 'Migrate WebP Images', 'ewww-image-optimizer-cloud' ); ?></h1>
	<?php
	...
	$button_text   = esc_attr__( 'Start Migration', 'ewww-image-optimizer-cloud' );
	...
	?>
		...
		<div id="webp-status"></div>
			...
			<form id="webp-start" class="webp-form" method="post" action="">
				<input id="webp-first" type="submit" class="button-secondary action" value="<?php esc_attr_e( 'Start Migration', 'ewww-image-optimizer-cloud' ); ?>" />
			</form>
	</div>
	<?php
}
...
function ewww_image_optimizer_webp_initialize() {
	...
	ewwwio_ob_clean();
	die( '<p>' . esc_html__( 'Scanning', 'ewww-image-optimizer-cloud' ) . '&nbsp;<img src="' . esc_url( $loading_image ) . '" /></p>' );
}
...
function ewww_image_optimizer_webp_loop() {
	...
	if ( $images ) {
		...
		printf( esc_html__( '%d Webp images left to rename.', 'ewww-image-optimizer-cloud' ), count( $images ) );
		echo '<br>';
	}
	while ( $images ) {
		$images_processed++;
...
function ewww_image_optimizer_webp_cleanup() {
	...
	ewwwio_ob_clean();
	if ( $skipped ) {
		echo '<p><b>' . esc_html__( 'Skipped:', 'ewww-image-optimizer-cloud' ) . '</b></p>';
		echo wp_kses_post( "<p>$skipped</p>" );
	}
	// and let the user know we are done.
	die( '<p><b>' . esc_html__( 'Finished', 'ewww-image-optimizer-cloud' ) . '</b></p>' );
}
