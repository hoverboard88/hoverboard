		...
		switch ( $library ) {
			case 'all':
				...
				if ( class_exists( 'EWWW_Nextgen' ) ) {
					...
					WP_CLI::line( 'Nextgen: ' . sprintf( __( '%1$d images have been selected, with %2$d resized versions.', 'ewww-image-optimizer-cloud' ), $fullsize_count, $resize_count ) );
				...
				if ( class_exists( 'EWWW_Flag' ) ) {
					...
					/* translators: 1-2: number of images */
					WP_CLI::line( 'FlAGallery: ' . sprintf( __( '%1$d images have been selected, with %2$d resized versions.', 'ewww-image-optimizer-cloud' ), $fullsize_count, $resize_count ) );
				}
			...
			case 'nextgen':
				...
				if ( class_exists( 'EWWW_Nextgen' ) ) {
					...
					if ( empty( $assoc_args['noprompt'] ) ) {
						/* translators: 1-2: number of images */
						WP_CLI::confirm( sprintf( __( '%1$d images have been selected, with %2$d resized versions.', 'ewww-image-optimizer-cloud' ), $fullsize_count, $resize_count ) );
					}
					ewww_image_optimizer_bulk_ngg( $delay );
				} elseif ( class_exists( 'EWWW_Nextcellent' ) ) {
					$attachments = ewww_image_optimizer_scan_next();
			...
			case 'flagallery':
				...
				if ( class_exists( 'EWWW_Flag' ) ) {
					...
					if ( empty( $assoc_args['noprompt'] ) ) {
						/* translators: 1-2: number of images */
						WP_CLI::confirm( 'FlAGallery: ' . sprintf( __( '%1$d images have been selected, with %2$d resized versions.', 'ewww-image-optimizer-cloud' ), $fullsize_count, $resize_count ) );
					}
					ewww_image_optimizer_bulk_flag( $delay );
				} else {
		...
		if ( ewww_image_optimizer_iterable( $ewwwngg->bulk_sizes ) ) {
			...
			foreach ( $ewwwngg->bulk_sizes as $size => $results_msg ) {
				if ( 'backup' === $size ) {
					...
					/* translators: %s: compression results */
					WP_CLI::line( sprintf( __( 'Full size - %s', 'ewww-image-optimizer-cloud' ), html_entity_decode( $results_msg ) ) );
				} elseif ( 'thumbnail' === $size ) {
					// Output the results of the thumb optimization.
					/* translators: %s: compression results */
					WP_CLI::line( sprintf( __( 'Thumbnail - %s', 'ewww-image-optimizer-cloud' ), html_entity_decode( $results_msg ) ) );
				} else {
...
function ewww_image_optimizer_bulk_next( $delay, $attachments ) {
	...
	foreach ( $attachments as $id ) {
		...
		if ( $fres[0] ) {
			// Output the results of the optimization.
			WP_CLI::line( __( 'Optimized image:', 'ewww-image-optimizer-cloud' ) . $fres[0] );
		}
		/* translators: %s: compression results */
		WP_CLI::line( sprintf( __( 'Full size - %s', 'ewww-image-optimizer-cloud' ), html_entity_decode( $fres[1] ) ) );
		// Output the results of the thumb optimization.
