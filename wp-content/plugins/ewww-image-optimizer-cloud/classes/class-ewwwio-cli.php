...
class EWWWIO_CLI extends WP_CLI_Command {
	...
	function optimize( $args, $assoc_args ) {
		...
		if ( ! empty( $assoc_args['force'] ) ) {
			WP_CLI::line( __( 'Forcing re-optimization of previously processed images.', 'ewww-image-optimizer-cloud' ) );
			$_REQUEST['ewww_force'] = true;
		}
		/* translators: 1: type of images, like media, or nextgen 2: number of seconds */
		WP_CLI::line( sprintf( _x( 'Optimizing %1$s with a %2$d second pause between images.', 'string will be something like "media" or "nextgen"', 'ewww-image-optimizer-cloud' ), $library, $delay ) );
		...
		switch ( $library ) {
			case 'all':
				if ( $ewww_reset ) {
					...
					WP_CLI::line( __( 'Bulk status has been reset, starting from the beginning.', 'ewww-image-optimizer-cloud' ) );
				}
				...
				WP_CLI::line( sprintf( _n( '%1$d image in the Media Library has been selected.', '%1$d images in the Media Library have been selected.', $fullsize_count, 'ewww-image-optimizer-cloud' ), $fullsize_count ) );
				WP_CLI::line( __( 'The active theme, BuddyPress, WP Symposium, and folders that you have configured will also be scanned for unoptimized images.', 'ewww-image-optimizer-cloud' ) );
				WP_CLI::line( __( 'Scanning, this could take a while', 'ewww-image-optimizer-cloud' ) );
				...
				if ( class_exists( 'EwwwNgg' ) ) {
					...
					if ( preg_match( '/^2/', $ngg->version ) ) {
						...
						WP_CLI::line( 'Nextgen: ' . sprintf( __( '%1$d images have been selected (%2$d unoptimized), with %3$d resizes (%4$d unoptimized).', 'ewww-image-optimizer-cloud' ), $fullsize_count, $unoptimized_count, $resize_count, $unoptimized_resize_count ) );
					} else {
						$attachments = ewww_image_optimizer_scan_next();
						/* translators: %d: number of images */
						WP_CLI::line( 'Nextgen: ' . sprintf( _n( 'There is %d image ready to optimize.', 'There are %d images ready to optimize.', count( $attachments ), 'ewww-image-optimizer-cloud' ), count( $attachments ) ) );
					}
				}
				if ( class_exists( 'ewwwflag' ) ) {
					...
					/* translators: 1-4: number of images */
					WP_CLI::line( 'Flagallery: ' . sprintf( __( '%1$d images have been selected (%2$d unoptimized), with %3$d resizes (%4$d unoptimized).', 'ewww-image-optimizer-cloud' ), $fullsize_count, $unoptimized_count, $resize_count, $unoptimized_resize_count ) );
				}
				if ( empty( $assoc_args['noprompt'] ) ) {
					/* translators: %d: number of images */
					WP_CLI::confirm( sprintf( _n( 'There is %d image ready to optimize.', 'There are %d images ready to optimize.', $pending_count, 'ewww-image-optimizer-cloud' ), $pending_count ) );
				}
			...
			case 'other':
				...
				$fullsize_count = ewww_image_optimizer_count_optimized( 'media' );
				/* translators: %d: number of images */
				WP_CLI::line( sprintf( __( '%1$d images in the Media Library have been selected.', 'ewww-image-optimizer-cloud' ), $fullsize_count ) );
				WP_CLI::line( __( 'The active theme, BuddyPress, WP Symposium, and folders that you have configured will also be scanned for unoptimized images.', 'ewww-image-optimizer-cloud' ) );
				WP_CLI::line( __( 'Scanning, this could take a while', 'ewww-image-optimizer-cloud' ) );
				// Do a filter to increase the timeout to 999 or something crazy.
				add_filter( 'ewww_image_optimizer_timeout', 'ewww_image_optimizer_cli_timeout', 200 );
			...
			case 'nextgen':
				...
				if ( class_exists( 'EwwwNgg' ) ) {
					...
					if ( preg_match( '/^2/', $ngg->version ) ) {
						...
						if ( empty( $assoc_args['noprompt'] ) ) {
							/* translators: 1-4: number of images */
							WP_CLI::confirm( sprintf( __( '%1$d images have been selected (%2$d unoptimized), with %3$d resizes (%4$d unoptimized).', 'ewww-image-optimizer-cloud' ), $fullsize_count, $unoptimized_count, $resize_count, $unoptimized_resize_count ) );
						}
						ewww_image_optimizer_bulk_ngg( $delay );
					} else {
						$attachments = ewww_image_optimizer_scan_next();
						if ( empty( $assoc_args['noprompt'] ) ) {
							/* translators: %d: number of images */
							WP_CLI::confirm( sprintf( _n( 'There is %d image ready to optimize.', 'There are %d images ready to optimize.', count( $attachments ), 'ewww-image-optimizer-cloud' ), count( $attachments ) ) );
						}
						ewww_image_optimizer_bulk_next( $delay, $attachments );
					}
				} else {
					WP_CLI::error( __( 'NextGEN/Nextcellent not installed.', 'ewww-image-optimizer-cloud' ) );
				}
				break;
			case 'flagallery':
				if ( $ewww_reset ) {
				...
				if ( class_exists( 'ewwwflag' ) ) {
					...
					WP_CLI::error( __( 'Grand Flagallery not installed.', 'ewww-image-optimizer-cloud' ) );
				}
				break;
			default:
				if ( $ewww_reset ) {
					...
					update_option( 'ewww_image_optimizer_bulk_ngg_resume', '' );
					update_option( 'ewww_image_optimizer_bulk_flag_resume', '' );
					WP_CLI::success( __( 'Bulk status has been reset, the next bulk operation will start from the beginning.', 'ewww-image-optimizer-cloud' ) );
				} else {
					WP_CLI::line( __( 'Please specify a valid library option, see "wp-cli help ewwwio optimize" for more information.', 'ewww-image-optimizer-cloud' ) );
				}
		} // End switch().
	}
}
...
function ewww_image_optimizer_bulk_media_cleanup() {
	...
	WP_CLI::success( __( 'Finished Optimization!', 'ewww-image-optimizer-cloud' ) );
}
...
function ewww_image_optimizer_bulk_flag( $delay = 0 ) {
	...
	if ( ! ewww_image_optimizer_iterable( $ids ) ) {
		WP_CLI::line( __( 'You do not appear to have uploaded any images yet.', 'ewww-image-optimizer-cloud' ) );
		return;
	}
	foreach ( $ids as $id ) {
		...
		WP_CLI::line( __( 'Optimized image:', 'ewww-image-optimizer-cloud' ) . ' ' . esc_html( $meta->image->filename ) );
		/* translators: %s: compression results */
		WP_CLI::line( sprintf( __( 'Full size – %s', 'ewww-image-optimizer-cloud' ), html_entity_decode( $fres[1] ) ) );
		if ( ! empty( $meta->image->meta_data['webview'] ) ) {
			...
			/* translators: %s: compression results */
			WP_CLI::line( sprintf( __( 'Optimized size – %s', 'ewww-image-optimizer-cloud' ), html_entity_decode( $wres[1] ) ) );
		}
		$thumb_path = $meta->image->thumbPath;
		// Optimize the thumbnail.
		$tres = ewww_image_optimizer( $thumb_path, 3, false, true );
		/* translators: %s: compression results */
		WP_CLI::line( sprintf( __( 'Thumbnail – %s', 'ewww-image-optimizer-cloud' ), html_entity_decode( $tres[1] ) ) );
...
function ewww_image_optimizer_bulk_ngg( $delay = 0 ) {
	...
	foreach ( $images as $id ) {
		...
		WP_CLI::line( __( 'Optimized image:', 'ewww-image-optimizer-cloud' ) . ' ' . basename( $storage->object->get_image_abspath( $image, 'full' ) ) );
		...
		if ( ewww_image_optimizer_iterable( $sizes ) ) {
			...
			foreach ( $sizes as $size ) {
				if ( 'full' === $size ) {
					/* translators: %s: compression results */
					WP_CLI::line( sprintf( __( 'Full size - %s', 'ewww-image-optimizer-cloud' ), html_entity_decode( $image->meta_data['ewww_image_optimizer'] ) ) );
				} elseif ( 'thumbnail' === $size ) {
					// Output the results of the thumb optimization.
					/* translators: %s: compression results */
					WP_CLI::line( sprintf( __( 'Thumbnail - %s', 'ewww-image-optimizer-cloud' ), html_entity_decode( $image->meta_data[ $size ]['ewww_image_optimizer'] ) ) );
				} else {
...
function ewww_image_optimizer_bulk_next( $delay, $attachments ) {
	...
	foreach ( $attachments as $id ) {
		...
		// Output the results of the optimization.
		WP_CLI::line( __( 'Optimized image:', 'ewww-image-optimizer-cloud' ) . $meta->image->filename );
		/* translators: %s: compression results */
		WP_CLI::line( sprintf( __( 'Full size - %s', 'ewww-image-optimizer-cloud' ), html_entity_decode( $fres[1] ) ) );
