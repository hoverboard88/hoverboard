...
if ( ! class_exists( 'EWWW_Nextgen' ) ) {
	...
	class EWWW_Nextgen {
		...
		function ewww_ngg_bulk_menu() {
			...
			add_submenu_page( NGGFOLDER, esc_html__( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ), esc_html__( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ), apply_filters( 'ewww_image_optimizer_bulk_permissions', '' ), 'ewww-ngg-bulk', array( &$this, 'ewww_ngg_bulk_preview' ) );
		...
		function ewww_ngg_manual() {
			...
			if ( false === isset( $_REQUEST['ewww_attachment_ID'] ) ) {
				...
				ewwwio_ob_clean();
				wp_die( wp_json_encode( array( 'error' => esc_html__( 'No attachment ID was provided.', 'ewww-image-optimizer-cloud' ) ) ) );
			}
		...
		function manage_images_columns( $columns ) {
			if ( is_array( $columns ) ) {
				$columns['ewww_image_optimizer'] = esc_html__( 'Image Optimizer', 'ewww-image-optimizer-cloud' );
			}
			return $columns;
		}
		...
		function ewww_manage_images_columns( $columns = null ) {
			...
			return esc_html__( 'Image Optimizer', 'ewww-image-optimizer-cloud' );
		}
				...
				switch ( $type ) {
					case 'image/jpeg':
						...
						if ( ! EWWW_IMAGE_OPTIMIZER_JPEGTRAN && ! $skip['jpegtran'] ) {
							...
							$msg = '<div>' . sprintf( esc_html__( '%s is missing', 'ewww-image-optimizer-cloud' ), '<em>jpegtran</em>' ) . '</div>';
						}
						break;
					case 'image/png':
						...
						if ( ( ! $skip['optipng'] && ! EWWW_IMAGE_OPTIMIZER_OPTIPNG ) || ( ! $skip['pngout'] && ! EWWW_IMAGE_OPTIMIZER_PNGOUT ) ) {
							/* translators: %s: name of a tool like jpegtran */
							$msg = '<div>' . sprintf( esc_html__( '%s is missing', 'ewww-image-optimizer-cloud' ), '<em>optipng/pngout</em>' ) . '</div>';
						}
						break;
					case 'image/gif':
						...
						if ( ! EWWW_IMAGE_OPTIMIZER_GIFSICLE && ! $skip['gifsicle'] ) {
							/* translators: %s: name of a tool like jpegtran */
							$msg = '<div>' . sprintf( esc_html__( '%s is missing', 'ewww-image-optimizer-cloud' ), '<em>gifsicle</em>' ) . '</div>';
						}
						break;
					default:
		...
		function ewww_ngg_bulk_preview() {
			...
			?>
			<div class="wrap">
				<h1><?php esc_html_e( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ); ?></h1>
				<?php
				if ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_cloud_key' ) ) {
					ewww_image_optimizer_cloud_verify( ewww_image_optimizer_get_option( 'ewww_image_optimizer_cloud_key' ) );
					echo '<a id="ewww-bulk-credits-available" target="_blank" class="page-title-action" style="float:right;" href="https://ewww.io/my-account/">' . esc_html__( 'Image credits available:', 'ewww-image-optimizer-cloud' ) . ' ' . esc_html( ewww_image_optimizer_cloud_quota() ) . '</a>';
				}
				...
				if ( empty( $resume ) ) {
					$button_text = __( 'Start optimizing', 'ewww-image-optimizer-cloud' );
				} else {
					$button_text = __( 'Resume previous optimization', 'ewww-image-optimizer-cloud' );
				}
				...
				$selected_images_text = sprintf( __( '%1$d images have been selected, with %2$d resized versions.', 'ewww-image-optimizer-cloud' ), $fullsize_count, $resize_count );
				?>
			...
			<div id="ewww-bulk-widgets" class="metabox-holder" style="display:none">
				...
				<div id="ewww-bulk-last" class="postbox">
					...
					<h2 class="ewww-hndle"><span><?php esc_html_e( 'Last Image Optimized', 'ewww-image-optimizer-cloud' ); ?></span></h2>
			...
			if ( ! empty( $resume ) ) {
				?>
				<p class="ewww-bulk-info" style="margin-top:3em;"><?php esc_html_e( 'Would like to clear the queue and start over?', 'ewww-image-optimizer-cloud' ); ?></p>
				<form id="ewww-bulk-reset" class="ewww-bulk-form" method="post" action="">
					...
					<input type="submit" class="button-secondary action" value="<?php esc_attr_e( 'Clear Queue', 'ewww-image-optimizer-cloud' ); ?>" />
				</form>
				<?php
			}
			echo '</div>';
			if ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_debug' ) ) {
				...
				echo '<p><strong>' . esc_html__( 'Debugging Information', 'ewww-image-optimizer-cloud' ) . ':</strong></p><div style="border:1px solid #e5e5e5;background:#fff;overflow:auto;height:300px;width:800px;">' . wp_kses_post( $eio_debug ) . '</div>';
			}
			echo '</div>';
			return;
		}
		...
		function ewww_ngg_bulk_script( $hook ) {
			...
			wp_localize_script(
				...
				array(
					...
					'scan_fail'             => esc_html__( 'Operation timed out, you may need to increase the max_execution_time for PHP', 'ewww-image-optimizer-cloud' ),
					...
					'temporary_failure'     => esc_html__( 'Temporary failure, seconds left to retry:', 'ewww-image-optimizer-cloud' ),
		...
		function ewww_ngg_bulk_init() {
			...
			if ( empty( $_REQUEST['ewww_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_REQUEST['ewww_wpnonce'] ), 'ewww-image-optimizer-bulk' ) || ! current_user_can( $permissions ) ) {
				$output['error'] = esc_html__( 'Access denied.', 'ewww-image-optimizer-cloud' );
				ewwwio_ob_clean();
			...
			if ( empty( $file ) ) {
				$output['results'] = '<p>' . esc_html__( 'Optimizing', 'ewww-image-optimizer-cloud' ) . "&nbsp;<img src='$loading_image' alt='loading'/></p>";
			} else {
				$output['results'] = '<p>' . esc_html__( 'Optimizing', 'ewww-image-optimizer-cloud' ) . " <b>$file</b>&nbsp;<img src='$loading_image' alt='loading'/></p>";
			}
			ewwwio_ob_clean();
			wp_die( wp_json_encode( $output ) );
		}
		...
		function ewww_ngg_bulk_loop() {
			...
			$output['results'] = sprintf( '<p>' . esc_html__( 'Optimized image:', 'ewww-image-optimizer-cloud' ) . ' <strong>%s</strong><br>', esc_html( basename( $storage->object->get_image_abspath( $image, 'full' ) ) ) );
			if ( ewww_image_optimizer_iterable( $this->bulk_sizes ) ) {
				foreach ( $this->bulk_sizes as $size => $results_msg ) {
					if ( 'backup' === $size ) {
						...
						$output['results'] .= sprintf( esc_html__( 'Full size - %s', 'ewww-image-optimizer-cloud' ) . '<br>', esc_html( $results_msg ) );
					} elseif ( 'thumbnail' === $size ) {
						// Output the results of the thumb optimization.
						/* Translators: %s: The compression results/savings */
						$output['results'] .= sprintf( esc_html__( 'Thumbnail - %s', 'ewww-image-optimizer-cloud' ) . '<br>', esc_html( $results_msg ) );
					} else {
			...
			$output['results']  .= sprintf( esc_html( _n( 'Elapsed: %s second', 'Elapsed: %s seconds', $elapsed, 'ewww-image-optimizer-cloud' ) ) . '</p>', number_format_i18n( $elapsed, 2 ) );
			$output['completed'] = 1;
			...
			if ( ! empty( $attachments ) ) {
				...
				$loading_image   = plugins_url( '/images/wpspin.gif', EWWW_IMAGE_OPTIMIZER_PLUGIN_FILE );
				if ( $next_file ) {
					$output['next_file'] = '<p>' . esc_html__( 'Optimizing', 'ewww-image-optimizer-cloud' ) . " <b>$next_file</b>&nbsp;<img src='$loading_image' alt='loading'/></p>";
				} else {
					$output['next_file'] = '<p>' . esc_html__( 'Optimizing', 'ewww-image-optimizer-cloud' ) . "&nbsp;<img src='$loading_image' alt='loading'/></p>";
				}
			} else {
				$output['done'] = 1;
			}
		...
		function ewww_ngg_bulk_cleanup() {
			...
			if ( empty( $_REQUEST['ewww_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_REQUEST['ewww_wpnonce'] ), 'ewww-image-optimizer-bulk' ) || ! current_user_can( $permissions ) ) {
				ewwwio_ob_clean();
				wp_die( esc_html__( 'Access token has expired, please reload the page.', 'ewww-image-optimizer-cloud' ) );
			}
			// Reset all the bulk options in the db.
			update_option( 'ewww_image_optimizer_bulk_ngg_resume', '' );
			update_option( 'ewww_image_optimizer_bulk_ngg_attachments', '', false );
			ewwwio_ob_clean();
			wp_die( '<p><b>' . esc_html__( 'Finished Optimization!', 'ewww-image-optimizer-cloud' ) . '</b></p>' );
		}
		...
		function ewww_ngg_bulk_actions_script() {
			...
			<script type="text/javascript">
				jQuery(document).ready(function($){
					$('select[name^="bulkaction"] option:last-child').after('<option value="bulk_optimize"><?php esc_html_e( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ); ?></option>');
				});
			</script>
			<?php
		}
