	...
	class EWWW_Nextgen {
		...
		function ewww_manage_image_custom_column( $column_name, $id ) {
			...
			if ( 'ewww_image_optimizer' === $column_name || ! $column_name ) {
				...
				if ( ! empty( $optimized_images ) ) {
					...
					echo esc_html( 'In Progress', 'ewww-image-optimizer-cloud' ) . '<br>';
		...
		function ewww_ngg_bulk_preview() {
			...
			<div class="wrap">
				...
				$selected_images_text = sprintf( __( '%1$d images have been selected (%2$d unoptimized), with %3$d resizes (%4$d unoptimized).', 'ewww-image-optimizer-cloud' ), $fullsize_count, $unoptimized_count, $resize_count, $unoptimized_resize_count );
				?>
			...
			<form class="ewww-bulk-form">
				<p><label for="ewww-force" style="font-weight: bold"><?php esc_html_e( 'Force re-optimize', 'ewww-image-optimizer-cloud' ); ?></label>&emsp;<input type="checkbox" id="ewww-force" name="ewww-force"></p>
				<p><label for="ewww-delay" style="font-weight: bold"><?php esc_html_e( 'Choose how long to pause between images (in seconds, 0 = disabled)', 'ewww-image-optimizer-cloud' ); ?></label>&emsp;<input type="text" id="ewww-delay" name="ewww-delay" value="<?php echo (int) $delay; ?>"></p>
			...
			if ( ! empty( $resume ) ) {
				?>
				<p class="ewww-bulk-info"><?php esc_html_e( 'If you would like to start over again, press the Reset Status button to reset the bulk operation status.', 'ewww-image-optimizer-cloud' ); ?></p>
				<form id="ewww-bulk-reset" class="ewww-bulk-form" method="post" action="">
					...
					<input type="submit" class="button-secondary action" value="<?php esc_attr_e( 'Reset Status', 'ewww-image-optimizer-cloud' ); ?>" />
				</form>
				<?php
			}
			echo '</div>';
			...
			if ( ewww_image_optimizer_iterable( $sizes ) ) {
				foreach ( $sizes as $size ) {
					if ( 'backup' === $size ) {
						...
						/* Translators: %s: The compression results/savings */
						$output['results'] .= sprintf( esc_html__( 'Thumbnail - %s', 'ewww-image-optimizer-cloud' ) . '<br>', esc_html( $image->meta_data[ $size ]['ewww_image_optimizer'] ) );
					} else {
