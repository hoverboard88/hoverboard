...
if ( ! class_exists( 'EWWW_Flag' ) ) {
	...
	class EWWW_Flag {
		...
		function ewww_flag_bulk_menu() {
			add_submenu_page( 'flag-overview', esc_html__( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ), esc_html__( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ), 'FlAG Manage gallery', 'flag-bulk-optimize', array( &$this, 'ewww_flag_bulk' ) );
		}
		...
		function ewww_manage_images_bulkaction() {
			echo '<option value="bulk_optimize_images">' . esc_html__( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ) . '</option>';
		}
		...
		function ewww_manage_galleries_bulkaction() {
			echo '<option value="bulk_optimize_galleries">' . esc_html__( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ) . '</option>';
		}
		...
		function ewww_flag_bulk() {
			...
			?>
			<div class="wrap"><h1>GRAND FlAGallery <?php esc_html_e( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ); ?></h1>
			<?php
			...
			if ( ! empty( $resume ) ) :
				?>
			<p class="ewww-bulk-info" style="margin-top:3em;"><?php esc_html_e( 'Would you like clear the queue and start over?', 'ewww-image-optimizer-cloud' ); ?></p>
			<form method="post" class="ewww-bulk-form" action="">
				...
				<button id="bulk-reset" type="submit" class="button-secondary action"><?php esc_html_e( 'Clear Queue', 'ewww-image-optimizer-cloud' ); ?></button>
			</form>
				<?php
			endif;
			echo '</div></div>';
		...
		function ewww_flag_bulk_init() {
			...
			if ( empty( $file_name ) ) {
				...
				$output['results'] = '<p>' . esc_html__( 'Optimizing', 'ewww-image-optimizer-cloud' ) . ' <b>' . $file_name . "</b>&nbsp;<img src='$loading_image' alt='loading'/></p>";
			}
			ewwwio_ob_clean();
			wp_die( wp_json_encode( $output ) );
		}
		...
		function ewww_flag_bulk_loop() {
			...
			$output['results'] = sprintf( '<p>' . esc_html__( 'Optimized image:', 'ewww-image-optimizer-cloud' ) . ' <strong>%s</strong><br>', esc_html( $meta->image->filename ) );
			/* Translators: %s: The compression results/savings */
			$output['results'] .= sprintf( esc_html__( 'Full size – %s', 'ewww-image-optimizer-cloud' ) . '<br>', esc_html( $fres[1] ) );
			if ( ! empty( $meta->image->meta_data['webview'] ) ) {
				// Determine path of the webview.
				$web_path           = $meta->image->webimagePath;
				$ewww_image         = new EWWW_Image( $id, 'flag', $web_path );
				...
				$wres               = ewww_image_optimizer( $web_path, 3, false, true );
				/* Translators: %s: The compression results/savings */
				$output['results'] .= sprintf( esc_html__( 'Optimized size – %s', 'ewww-image-optimizer-cloud' ) . '<br>', esc_html( $wres[1] ) );
			}
			$thumb_path         = $meta->image->thumbPath;
			$ewww_image         = new EWWW_Image( $id, 'flag', $thumb_path );
			$ewww_image->resize = 'thumbnail';
			...
			/* Translators: %s: The compression results/savings */
			$output['results'] .= sprintf( esc_html__( 'Thumbnail – %s', 'ewww-image-optimizer-cloud' ) . '<br>', esc_html( $tres[1] ) );
		...
		function ewww_manage_image_custom_column( $column_name, $id ) {
			...
			if ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_debug' ) && ewww_image_optimizer_function_exists( 'print_r' ) ) {
				...
				echo '<button type="button" class="ewww-show-debug-meta button button-secondary" data-id="' . (int) $id . '">' .
					esc_html__( 'Show Metadata', 'ewww-image-optimizer-cloud' ) .
			...
			switch ( $type ) {
				case 'image/jpeg':
					if ( ! EWWW_IMAGE_OPTIMIZER_JPEGTRAN && ! $skip['jpegtran'] ) {
						...
						echo '<div>' . sprintf( esc_html__( '%s is missing', 'ewww-image-optimizer-cloud' ), '<em>jpegtran</em>' ) . '</div></div>';
						return;
					}
					break;
				case 'image/png':
					if ( ( ! $skip['optipng'] && ! EWWW_IMAGE_OPTIMIZER_OPTIPNG ) || ( ! $skip['pngout'] && ! EWWW_IMAGE_OPTIMIZER_PNGOUT ) ) {
						/* translators: %s: name of a tool like jpegtran */
						echo '<div>' . sprintf( esc_html__( '%s is missing', 'ewww-image-optimizer-cloud' ), '<em>optipng/pngout</em>' ) . '</div></div>';
						return;
					}
					break;
				case 'image/gif':
					if ( ! EWWW_IMAGE_OPTIMIZER_GIFSICLE && ! $skip['gifsicle'] ) {
						/* translators: %s: name of a tool like jpegtran */
						echo '<div>' . sprintf( esc_html__( '%s is missing', 'ewww-image-optimizer-cloud' ), '<em>gifsicle</em>' ) . '</div></div>';
						return;
					}
					break;
				default:
					echo '<div>' . esc_html__( 'Unsupported file type', 'ewww-image-optimizer-cloud' ) . '</div></div>';
					return;
			}
