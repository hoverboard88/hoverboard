...
if ( ! class_exists( 'EWWW_Nextcellent' ) ) {
	...
	class EWWW_Nextcellent {
		...
		function ewww_ngg_bulk_menu() {
			add_submenu_page( NGGFOLDER, esc_html__( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ), esc_html__( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ), 'NextGEN Manage gallery', 'ewww-ngg-bulk', array( &$this, 'ewww_ngg_bulk_preview' ) );
		}
				...
				switch ( $type ) {
					...
					case 'image/png':
						...
						if ( ! EWWW_IMAGE_OPTIMIZER_OPTIPNG && ! $skip['optipng'] ) {
							/* translators: %s: name of a tool like jpegtran */
							echo '<div>' . sprintf( esc_html__( '%s is missing', 'ewww-image-optimizer-cloud' ), '<em>optipng</em>' ) . '</div></div>';
							if ( $return ) {
								return ob_get_clean();
							}
							return;
		...
		function ewww_ngg_bulk_preview() {
			...
			<div class="wrap">
				...
				if ( empty( $resume ) ) {
					...
					$button_text = __( 'Resume previous bulk operation', 'ewww-image-optimizer-cloud' );
				}
				/* translators: %d: number of images */
				$selected_images_text = sprintf( _n( 'There is %d image ready to optimize.', 'There are %d images ready to optimize.', count( $attachments ), 'ewww-image-optimizer-cloud' ), count( $attachments ) );
				?>
		...
		function ewww_ngg_bulk_loop() {
			...
			$permissions = apply_filters( 'ewww_image_optimizer_bulk_permissions', '' );
			if ( empty( $_REQUEST['ewww_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_REQUEST['ewww_wpnonce'] ), 'ewww-image-optimizer-bulk' ) || ! current_user_can( $permissions ) ) {
				$outupt['error'] = esc_html__( 'Access token has expired, please reload the page.', 'ewww-image-optimizer-cloud' );
				ewwwio_ob_clean();
				wp_die( wp_json_encode( $output ) );
			}
			session_write_close();
			...
			if ( $fres[0] ) {
				$output['results'] = sprintf( '<p>' . esc_html__( 'Optimized image:', 'ewww-image-optimizer-cloud' ) . ' <strong>%s</strong><br>', esc_html( $fres[0] ) );
			}
