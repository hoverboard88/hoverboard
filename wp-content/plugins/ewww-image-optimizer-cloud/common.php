...
function ewww_image_optimizer_notice_exactdn_domain_mismatch() {
	...
	?>
		...
		printf(
			...
			'<a href="' . esc_url( admin_url( 'options-general.php?page=ewww-image-optimizer-options' ) ) . '">' . esc_html__( 'settings page', 'ewww-image-optimizer-cloud' ) . '</a>'
		);
		?>
		</p>
	</div>
...
function ewww_image_optimizer_settings_link( $links ) {
	...
	if ( is_multisite() && is_plugin_active_for_network( EWWW_IMAGE_OPTIMIZER_PLUGIN_FILE_REL ) ) {
		$settings_link = '<a href="' . network_admin_url( 'settings.php?page=ewww-image-optimizer-options' ) . '">' . esc_html__( 'Settings', 'ewww-image-optimizer-cloud' ) . '</a>';
	} else {
...
function ewww_image_optimizer_notice_invalid_key() {
	echo "<div id='ewww-image-optimizer-invalid-key' class='notice error'><p><strong>" . esc_html__( 'Could not validate EWWW Image Optimizer API key, please check your key to ensure it is correct.', 'ewww-image-optimizer-cloud' ) . '</strong></p></div>';
}
...
function ewww_image_optimizer_columns( $defaults ) {
	ewwwio_debug_message( '<b>' . __FUNCTION__ . '()</b>' );
	$defaults['ewww-image-optimizer-cloud'] = esc_html__( 'Image Optimizer', 'ewww-image-optimizer-cloud' );
	ewwwio_memory( __FUNCTION__ );
	return $defaults;
}
...
function ewww_image_optimizer_webp_rewrite() {
	...
	if ( $ewww_rules ) {
		if ( insert_with_markers( ewww_image_optimizer_htaccess_path(), 'EWWWIO', $ewww_rules ) && ! ewww_image_optimizer_webp_rewrite_verify() ) {
			esc_html_e( 'Insertion successful', 'ewww-image-optimizer-cloud' );
		} else {
			esc_html_e( 'Insertion failed', 'ewww-image-optimizer-cloud' );
		}
	}
	wp_die();
}
...
<div id='ewww-settings-wrap' class='wrap'>
	...
	if ( ! $easymode ) {
		echo '<p>' . sprintf(
			...
			esc_html__( 'New images uploaded to the Media Library will be optimized automatically. If you have existing images you would like to optimize, you can use the %s tool.', 'ewww-image-optimizer-cloud' ),
			( 'network-multisite' === esc_attr( $network ) ?
				esc_html__( 'Media Library', 'ewww-image-optimizer-cloud' ) . ' -> ' . esc_html__( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ) :
				'<a href="' . esc_url( admin_url( 'upload.php?page=ewww-image-optimizer-bulk' ) ) . '">' . esc_html__( 'Bulk Optimize', 'ewww-image-optimizer-cloud' ) . '</a>'
				)
		);
					...
					<div class="ewww-row ewww-blocks">
						<div id="ewww-compress" class="ewww-status-detail">
							...
							<div id="ewww-compress-recommend" class="ewww-recommend">
								<strong><?php echo ( (int) $speed_score < 100 ? esc_html__( 'How do I get to 100%?', 'ewww-image-optimizer-cloud' ) : esc_html__( 'You got the perfect score!', 'ewww-image-optimizer-cloud' ) ); ?></strong>
	...
	<?php if ( $speed_score < 100 ) : ?>
							<p class="description"><?php esc_html_e( 'Hover over score for recommendations to improve the speed of your site.', 'ewww-image-optimizer-cloud' ); ?></p>
	<?php endif; ?>
	...
	if ( $total_savings > 0 ) {
		...
		?>
						<div id="ewww-savings" class="ewww-status-detail">
							...
							<p style="text-align:center"><strong><?php esc_html_e( 'Local Compression Savings', 'ewww-image-optimizer-cloud' ); ?></strong></p>
							<p><a href="<?php echo esc_url( admin_url( 'tools.php?page=ewww-image-optimizer-tools' ) ); ?>"><?php esc_html_e( 'View optimized images.', 'ewww-image-optimizer-cloud' ); ?></a></p>
						</div><!-- end .ewww-status-detail -->
	<?php } ?>
	<?php
	...
	<?php if ( ! ewww_image_optimizer_get_option( 'ewww_image_optimizer_cloud_key' ) && ! ewww_image_optimizer_get_option( 'ewww_image_optimizer_exactdn' ) ) : ?>
							<h3>
								<a href='https://ewww.io/plans/' target='_blank' class='ewww-upgrade'><?php esc_html_e( 'Premium Upgrades:', 'ewww-image-optimizer-cloud' ); ?></a>
							</h3>
							<p><i><?php esc_html_e( 'Priority Support included with any purchase.', 'ewww-image-optimizer-cloud' ); ?></i></p>
	<?php endif; ?>
	<?php if ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_cloud_key' ) ) : ?>
							<p><strong><?php esc_html_e( 'Compress API Key', 'ewww-image-optimizer-cloud' ); ?></strong>
	...
	<?php else : ?>
		...
		<?php if ( $exactdn_enabled || get_option( 'easyio_exactdn' ) ) : ?>
									<?php esc_html_e( 'Need to save storage space?', 'ewww-image-optimizer-cloud' ); ?>
		<?php else : ?>
									<?php esc_html_e( 'Unlock premium compression, save storage space, and reduce server load.', 'ewww-image-optimizer-cloud' ); ?>
		<?php endif; ?>
								</a>
							</p>
	<?php endif; ?>
	<?php if ( class_exists( 'Jetpack_Photon' ) && Jetpack::is_module_active( 'photon' ) && ewww_image_optimizer_get_option( 'ewww_image_optimizer_exactdn' ) ) : ?>
							<p><b>Easy IO:</b> <span style="color: red"><?php esc_html_e( 'Inactive, please disable the Image Performance option on the Jetpack Dashboard.', 'ewww-image-optimizer-cloud' ); ?></span></p>
	...
	<?php elseif ( ! ewww_image_optimizer_get_option( 'ewww_image_optimizer_exactdn' ) && ! get_option( 'easyio_exactdn' ) ) : ?>
							<p><span style="font-weight:bold;color:#3eadc9;">Easy IO:</span> <a href="https://ewww.io/easy/" target="_blank"><?php esc_html_e( 'Comprehensive image optimization with auto-compress, auto-sizing, auto-WebP, and lazy load.', 'ewww-image-optimizer-cloud' ); ?></a></p>
	<?php endif; ?>
	<?php if ( ! function_exists( 'swis' ) ) : ?>
							<p><span style="font-weight:bold;color:#3eadc9;">SWIS Performance:</span> <a href="https://ewww.io/swis/" target="_blank"><?php esc_html_e( 'Go beyond image optimization with the tools I use for improving site speed.', 'ewww-image-optimizer-cloud' ); ?></a></p>
	<?php endif; ?>
							<p>
	...
	<ul class='ewww-tab-nav'>
		...
		<li class='ewww-tab ewww-exactdn-nav'><span><?php esc_html_e( 'Easy Mode', 'ewww-image-optimizer-cloud' ); ?></span></li>
		<?php endif; ?>
		<li class='ewww-tab ewww-optimization-nav'><span><?php esc_html_e( 'Advanced', 'ewww-image-optimizer-cloud' ); ?></span></li>
		...
		<li class='ewww-tab ewww-webp-nav'><span><?php esc_html_e( 'WebP', 'ewww-image-optimizer-cloud' ); ?></span></li>
	...
	<?php if ( 'network-singlesite' === $network ) : ?>
		<i class="network-singlesite"><strong><?php esc_html_e( 'Configure network-wide settings in the Network Admin.', 'ewww-image-optimizer-cloud' ); ?></strong></i>
		<?php ob_start(); ?>
	<?php endif; ?>
	...
	<?php if ( 'singlesite' === $network && $easymode ) : ?>
			<p><strong><?php esc_html_e( 'Easy IO is optimizing your site, no more configuration needed!', 'ewww-image-optimizer-cloud' ); ?></strong></p>
			<?php /* translators: %s: Local+Advanced Mode */ ?>
			<p><?php printf( esc_html__( 'If you need to save storage space, enable %s.', 'ewww-image-optimizer-cloud' ), '<a href="' . esc_url( wp_nonce_url( admin_url( 'options-general.php?page=ewww-image-optimizer-options&enable-local=1' ), 'ewww_image_optimizer_options-options' ) ) . '">' . esc_html__( 'Local+Advanced Mode', 'ewww-image-optimizer-cloud' ) . '</a>' ); ?></p>
	...
	<?php elseif ( 'singlesite' === $network && ewww_image_optimizer_get_option( 'ewww_image_optimizer_exactdn' ) ) : ?>
			<p><strong><?php esc_html_e( 'Easy IO is optimizing your site, no more configuration needed!', 'ewww-image-optimizer-cloud' ); ?></strong><br>
		<?php if ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_cloud_key' ) ) : ?>
			<?php /* translators: %s: Easy Mode */ ?>
			<?php printf( esc_html__( 'Remove API key and switch to %s.', 'ewww-image-optimizer-cloud' ), '<a href="' . esc_url( wp_nonce_url( admin_url( 'options-general.php?page=ewww-image-optimizer-options&enable-local=0' ), 'ewww_image_optimizer_options-options' ) ) . '">' . esc_html__( 'Easy Mode', 'ewww-image-optimizer-cloud' ) . '</a>' ); ?></p>
		<?php else : ?>
			<?php /* translators: %s: Easy Mode */ ?>
			<?php printf( esc_html__( 'Switch to %s.', 'ewww-image-optimizer-cloud' ), '<a href="' . esc_url( wp_nonce_url( admin_url( 'options-general.php?page=ewww-image-optimizer-options&enable-local=0' ), 'ewww_image_optimizer_options-options' ) ) . '">' . esc_html__( 'Easy Mode', 'ewww-image-optimizer-cloud' ) . '</a>' ); ?></p>
		<?php endif; ?>
	<?php elseif ( 'singlesite' === $network && get_option( 'easyio_exactdn' ) ) : ?>
			<p><strong><?php esc_html_e( 'Easy IO is optimizing your site, no more configuration needed!', 'ewww-image-optimizer-cloud' ); ?></strong><br>
			<?php esc_html_e( 'Unless you need to save storage space, you can now disable the EWWW Image Optimizer.', 'ewww-image-optimizer-cloud' ); ?></p>
	<?php endif; ?>
	<?php ob_start(); ?>
	...
	<?php if ( 'network-multisite-over' === $network ) : ?>
				<tr>
					...
					<td>
						...
						<?php esc_html_e( 'Allow EWWW Image Optimizer to anonymously track how this plugin is used and help us make the plugin better. Opt-in to tracking and receive 500 API image credits free. No sensitive data is tracked.', 'ewww-image-optimizer-cloud' ); ?>
					</td>
				</tr>
			</table>
	...
	<?php else : ?>
				<tr>
					...
					<td>
						...
						<?php esc_html_e( 'API Key will be validated when you save your settings.', 'ewww-image-optimizer-cloud' ); ?>
						<a href='https://optimize.exactlywww.com/api_key/free-trial.php?TB_iframe=true&width=600&height=375' class='thickbox' target='_blank'>
							<?php esc_html_e( 'Get a free trial key.', 'ewww-image-optimizer-cloud' ); ?>
						</a>
					</td>
				</tr>
	<?php endif; ?>
	...
	<?php if ( ! ewww_image_optimizer_get_option( 'ewww_image_optimizer_cloud_key' ) ) : ?>
						<p>
							* <strong><a href='https://ewww.io/buy-credits/' target='_blank'>
								<?php esc_html_e( 'Purchase an API key to unlock these optimization levels and get priority support. Achieve up to 80% compression to speed up your site, save storage space, and reduce server load.', 'ewww-image-optimizer-cloud' ); ?>
							</a></strong><br>
							<a href='https://optimize.exactlywww.com/api_key/free-trial.php?TB_iframe=true&width=600&height=375' class='thickbox' target='_blank'>
								<?php esc_html_e( 'Or get a free trial key and take it for a test drive.', 'ewww-image-optimizer-cloud' ); ?>
							</a>
						</p>
	<?php else : ?>
						<p>
	...
	?>
		...
		<div id='ewww-exactdn-settings'>
			<noscript><h2><?php esc_html_e( 'Easy Mode', 'ewww-image-optimizer-cloud' ); ?></h2></noscript>
			<p>
				<?php esc_html_e( 'Having problems? Try disabling Lazy Load and Include All Resources. Finally, disable Easy IO if problems remain.', 'ewww-image-optimizer-cloud' ); ?><br>
				<a class='ewww-docs-root' href='https://ewww.io/contact-us/'>
					<?php esc_html_e( 'Then, let us know so we can find a fix for the problem.', 'ewww-image-optimizer-cloud' ); ?>
				</a>
			...
			<table class='form-table'>
				<tr>
					...
					<td>
						...
						<?php esc_html_e( 'Comprehensive and automatic image optimization:', 'ewww-image-optimizer-cloud' ); ?>
						<p class="description">
							<?php esc_html_e( 'Premium Compression', 'ewww-image-optimizer-cloud' ); ?><br>
							<?php esc_html_e( 'Properly scale images to fit any device', 'ewww-image-optimizer-cloud' ); ?><br>
							<?php esc_html_e( 'Automatic WebP Conversion', 'ewww-image-optimizer-cloud' ); ?><br>
							<?php esc_html_e( 'Smart Image Conversion', 'ewww-image-optimizer-cloud' ); ?><br>
							<?php esc_html_e( 'JS/CSS Minification and Compression', 'ewww-image-optimizer-cloud' ); ?><br>
							<?php esc_html_e( 'CDN Included', 'ewww-image-optimizer-cloud' ); ?><br>
	...
	<?php endif; ?>
							<a href="https://docs.ewww.io/article/44-introduction-to-exactdn" target="_blank" data-beacon-article="59bc5ad6042863033a1ce370">
								<?php esc_html_e( 'Learn more about Easy IO', 'ewww-image-optimizer-cloud' ); ?>
							</a>
						</p>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						...
						<?php ewwwio_help_link( 'https://docs.ewww.io/article/47-getting-more-from-exactdn', '59de6631042863379ddc953c' ); ?>
						<p class='description'>
							<?php esc_html_e( 'Enable high quality premium compression for all images. Disable to use Pixel Perfect mode instead.', 'ewww-image-optimizer-cloud' ); ?>
						</p>
					</td>
				</tr>
	<?php if ( ! $exactdn_enabled || 1 === $exactdn->get_plan_id() ) : ?>
	...
	<?php endif; ?>
				...
				<tr>
					...
					<td>
						...
						<p class='description'>
							<?php esc_html_e( 'When used with Easy IO and/or JS WebP Rewriting, the plugin will load the best available image size and format for each device.', 'ewww-image-optimizer-cloud' ); ?></br>
			...
			<table class='form-table'>
				<tr>
					...
					<td>
						...
						<p class='description'>
							<?php esc_html_e( 'Use this to override the default WordPress quality level of 82. Applies to image editing, resizing, PNG to JPG conversion, and JPG to WebP conversion. Does not affect the original uploaded image unless maximum dimensions are set and resizing occurs.', 'ewww-image-optimizer-cloud' ); ?>
						</p>
					</td>
				</tr>
				<tr>
	...
	<?php endif; ?>
				<tr>
					...
					<td>
						...
						<p class='description'>
							<?php esc_html_e( 'Resize uploaded images to these dimensions.', 'ewww-image-optimizer-cloud' ); ?>
						</p>
					</td>
				</tr>
	<?php if ( ! $easymode ) : ?>
							...
							<tr>
								...
								<td>
									<label for='ewww_image_optimizer_disable_resizes_$size'>
										<?php echo esc_attr( $size ); ?> - <span class='description'><?php esc_html_e( 'Disabling creation of the full-size preview for PDF files will disable all PDF preview sizes', 'ewww-image-optimizer-cloud' ); ?></span>
									</label>
								</td>
							</tr>
			<?php else : ?>
	...
	<?php
	$rewrite_notice     = ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_webp' ) && ! ewww_image_optimizer_get_option( 'ewww_image_optimizer_webp_for_cdn' ) && ! ewww_image_optimizer_get_option( 'ewww_image_optimizer_picture_webp' ) ? __( 'You can use the rewrite rules below to serve WebP images with Apache.', 'ewww-image-optimizer-cloud' ) : '' );
	...
	<?php if ( ! $free_exec && ! ewww_image_optimizer_easy_active() ) : ?>
			<p>
				<?php esc_html_e( 'Once WebP conversion is enabled, WebP images will be generated for new uploads, but you will need to use the Bulk Optimizer for existing uploads.', 'ewww-image-optimizer-cloud' ); ?><br>
				<a href='https://ewww.io/easy/'><?php esc_html_e( 'Get Easy IO for automatic WebP conversion and delivery.', 'ewww-image-optimizer-cloud' ); ?></a>
			</p>
	<?php endif; ?>
			<table class='form-table'>
	<?php if ( $free_exec ) : ?>
	...
	<?php elseif ( ! ewww_image_optimizer_easy_active() || ewww_image_optimizer_get_option( 'ewww_image_optimizer_webp' ) ) : ?>
				<tr>
					...
					<td>
						...
						<p class='description'>
							<?php echo ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_cloud_key' ) ? '' : esc_html__( 'GIF to WebP conversion requires an active API key.', 'ewww-image-optimizer-cloud' ) ); ?>
	...
	<?php if ( ! $free_exec && ( ! ewww_image_optimizer_ce_webp_enabled() && ! ewww_image_optimizer_easy_active() ) ) : ?>
				<tr>
					<th scope='row'>
						<label for='ewww_image_optimizer_webp_for_cdn'><?php esc_html_e( 'JS WebP Rewriting', 'ewww-image-optimizer-cloud' ); ?></label>
					...
					<td>
						...
						<span><?php esc_html_e( 'Use this if the Apache rewrite rules do not work, or if your images are served from a CDN.', 'ewww-image-optimizer-cloud' ); ?></span>
						<p class='description'><?php esc_html_e( 'Supports CSS background images with Lazy Load option on the Easy Mode tab.', 'ewww-image-optimizer-cloud' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope='row'>
						<label for='ewww_image_optimizer_picture_webp'><?php esc_html_e( '<picture> WebP Rewriting', 'ewww-image-optimizer-cloud' ); ?></label>
						<span><?php ewwwio_help_link( 'https://docs.ewww.io/article/16-ewww-io-and-webp-images', '5854745ac697912ffd6c1c89,59443d162c7d3a0747cdf9f0' ); ?></span>
					</th>
					<td>
						...
						<span><?php esc_html_e( 'A JavaScript-free rewriting method using picture tags.', 'ewww-image-optimizer-cloud' ); ?></span>
						<p class='description'><?php esc_html_e( 'Use this if Apache rewrite rules do not work for your site. Some themes may not display <picture> tags properly, and does not support CSS background images.', 'ewww-image-optimizer-cloud' ); ?></p>
					</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
					<td>
						<label for='ewww_image_optimizer_webp_rewrite_exclude'><?php esc_html_e( 'JS WebP and <picture> Web Exclusions', 'ewww-image-optimizer-cloud' ); ?></label><br>
				...
				<tr>
					<th scope='row'>
						<label for='ewww_image_optimizer_webp_paths'><?php esc_html_e( 'WebP URLs', 'ewww-image-optimizer-cloud' ); ?></label>
						<span><?php ewwwio_help_link( 'https://docs.ewww.io/article/16-ewww-io-and-webp-images', '5854745ac697912ffd6c1c89' ); ?></span>
					</th>
					<td>
	...
	<?php endif; ?>
		...
		<div id='ewww-support-settings'>
			...
			<p>
				...
				<a href='https://ewww.io/chat/'><?php esc_html_e( 'Community Chat', 'ewww-image-optimizer-cloud' ); ?></a> |
			...
			<table class='form-table'>
				<tr>
					...
					<td>
						...
						<span><?php esc_html_e( 'Allow EWWW Image Optimizer to anonymously track how this plugin is used and help us make the plugin better. Opt-in to tracking and receive 500 API image credits free. No sensitive data is tracked.', 'ewww-image-optimizer-cloud' ); ?></span>
					</td>
				</tr>
			</table>
		</div>
	...
	<form id='ewww-webp-rewrite'>
		...
		<p id='ewww-webp-rewrite-status'><strong><?php esc_html_e( 'Rules verified successfully', 'ewww-image-optimizer-cloud' ); ?></strong></p>
...
AddType image/webp .webp</pre>
		...
		<p id='ewww-webp-rewrite-status'><?php esc_html_e( 'The image to the right will display a WebP image with WEBP in white text, if your site is serving WebP images and your browser supports WebP.', 'ewww-image-optimizer-cloud' ); ?></p>
	...
	echo '<hr style="clear:both;"><div id="ewwwio-banner"><img src="' . esc_url( plugins_url( '/images/ewwwio-logo.png', __FILE__ ) ) . '">' .
		'<p>' . esc_html__( 'Get performance tips, exclusive discounts, and the latest news when you signup for our newsletter!', 'ewww-image-optimizer-cloud' ) . '</p>' .
		'<a href="https://eepurl.com/gKyU6L?TB_iframe=true&width=600&height=610" class="thickbox button-secondary">' . esc_html__( 'Subscribe now!', 'ewww-image-optimizer-cloud' ) . ' &rarr;</a></div>';

	echo '<h2>' . esc_html__( "Shane's Recommendations", 'ewww-image-optimizer-cloud' ) . '</h2>';
	echo '<p>' . esc_html__( 'These are products I have personally used. An * indicates an affiliate link so you can support future development of EWWW IO.', 'ewww-image-optimizer-cloud' ) . '</p>';
	echo '<h3>' . esc_html__( 'Hosting', 'ewww-image-optimizer-cloud' ) . '</h3>' .
		'<p><a href="http://shareasale.com/r.cfm?b=917225&amp;u=1481701&amp;m=41388&amp;urllink=&amp;afftrack=">WP Engine</a>* - ' .
			esc_html__( 'ewww.io is powered by WP Engine, and their performance is worth every penny. This is "managed WordPress hosting" at it\'s finest. 20% discount off your first payment with our link.', 'ewww-image-optimizer-cloud' ) .
		'<br><a href="https://kinsta.com?kaid=OCKKBJARSDFR">Kinsta</a>* - ' .
			esc_html__( 'Their performance is the same blazing speed as WP Engine, and the support is top notch as well. In addition, they offer SSH access with WP-CLI. Very solid offering and highly recommended.', 'ewww-image-optimizer-cloud' ) .
		'<br><a href="https://vrlps.co/kXJu5yF/cp">Cloudways</a>* - ' .
			esc_html__( 'Simple, yet powerful, managed VPS hosting. I use their hosting for a few client sites and performance is crazy fast. Support has been great any time I\'ve needed their assistance.', 'ewww-image-optimizer-cloud' ) .
		'</p>' .
		'<h3>' . esc_html__( 'Plugins and Services', 'ewww-image-optimizer-cloud' ) . '</h3>' .
		'<p><a href="https://easydigitaldownloads.com/?ref=4735">Easy Digital Downloads</a>* - ' .
			esc_html__( 'Sell digital products with a huge number of extensions for amazing flexibility. Naturally, this is how we sell our services and plugins.', 'ewww-image-optimizer-cloud' ) .
		'<br><a href="https://www.helpscout.net/referral/?code=SVhaTmZDSSt2WG4xYS9FRmZrNllUa3RSR25VTStXcGhmaStQQ21QQjBDVmdZdz09OnoyVTFIUzBnbldtM21VMEM.">Helpscout</a>* - ' .
			esc_html__( 'Treat your customers like people, not numbers, and keep them satisfied. Personal favorite feature is Traffic Cop that keeps you from sending a message if you receive another message from the customer while you are typing.', 'ewww-image-optimizer-cloud' ) .
		'<br><a href="http://eepurl.com/cLPIeD">MailChimp</a>* - ' .
			esc_html__( 'Mailing lists that just work plus automation features that work even when you\'re not.', 'ewww-image-optimizer-cloud' ) .
		'<br><a href="https://malcare.com?src=C600E7">MalCare</a>* - ' .
			esc_html__( 'Secure your site with a firewall, advanced heuristic scanning, and brute force login protection. Securing your site is not an option, it\'s a necessity.', 'ewww-image-optimizer-cloud' ) .
		'<br><a href="https://nodeping.com?rid=201407082225W862K">NodePing</a>* - ' .
			esc_html__( 'Monitor all the things, and make sure they stay online. They have a fantastic array of monitors and notifications that you can setup, they are cheap, simple, and have great support.', 'ewww-image-optimizer-cloud' ) .
		'</p>';
