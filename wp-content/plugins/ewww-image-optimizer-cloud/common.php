...
function ewww_image_optimizer_manual() {
	...
	if ( get_transient( 'ewww_image_optimizer_cloud_status' ) === 'exceeded' || ewww_image_optimizer_get_option( 'ewww_image_optimizer_cloud_exceeded' ) > time() ) {
		if ( ! wp_doing_ajax() ) {
			wp_die( esc_html__( 'License exceeded', 'ewww-image-optimizer-cloud' ) );
		}
		ewwwio_ob_clean();
		wp_die( wp_json_encode( array( 'error' => esc_html__( 'License exceeded', 'ewww-image-optimizer-cloud' ) ) ) );
	}
...
function ewww_image_optimizer_aux_images_loop( $attachment = null, $auto = false, $cli = false ) {
	...
	if ( ! empty( $ewww_status ) && preg_match( '/exceeded/', $ewww_status ) ) {
		if ( ! $auto ) {
			$output['error'] = esc_html__( 'License Exceeded', 'ewww-image-optimizer-cloud' );
...
<div id='ewww-settings-wrap' class='wrap'>
	...
	if ( $total_savings > 0 ) {
		...
		?>
						<li><div id="ewww-savings" class="ewww-status-detail">
							...
							<p style="text-align:center"><strong><?php esc_html_e( 'Savings', 'ewww-image-optimizer-cloud' ); ?></strong></p>
	...
	<?php elseif ( ! ewww_image_optimizer_get_option( 'ewww_image_optimizer_exactdn' ) && ! get_option( 'easyio_exactdn' ) ) : ?>
							<p><span style="font-weight:bold;color:#3eadc9;">Easy IO:</span> <a href="https://ewww.io/easy/" target="_blank"><?php esc_html_e( 'Comprehensive image optimization with automatic compression, auto-sizing, WebP conversion, and lazy load.', 'ewww-image-optimizer-cloud' ); ?></a></p>
	<?php endif; ?>
	<?php if ( ! function_exists( 'swis' ) ) : ?>
							<p><span style="font-weight:bold;color:#3eadc9;">SWIS Performance:</span> <a href="https://ewww.io/swis/" target="_blank"><?php esc_html_e( 'Go beyond image optimization with this collection of tools that I use for improving site speed.', 'ewww-image-optimizer-cloud' ); ?></a></p>
	<?php endif; ?>
							<p>
