...
function ewww_image_optimizer_notice_exactdn_activation_success() {
	echo '<div id="ewww-image-optimizer-notice-exactdn-success" class="notice notice-success"><p>' .
		esc_html__( 'Easy IO setup and verification is complete.', 'ewww-image-optimizer-cloud' ) .
		'</p></div>';
}
...
<div id='ewww-settings-wrap' class='wrap'>
	...
	if ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_cloud_key' ) ) {
		...
		if ( false !== strpos( $verify_cloud, 'great' ) ) {
			...
			if ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_jpg_level' ) > 20 ) {
				...
				$compress_recommendations[] = __( 'Enable premium compression for JPG images.', 'ewww-image-optimizer-cloud' );
			}
			if ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_png_level' ) > 20 ) {
				$compress_score += 20;
			} else {
				$compress_recommendations[] = __( 'Enable premium compression for PNG images.', 'ewww-image-optimizer-cloud' );
			}
		}
		$disable_level = false;
	} else {
		...
		if ( ! class_exists( 'ExactDN' ) && ! ewww_image_optimizer_get_option( 'ewww_image_optimizer_exactdn' ) ) {
			$compress_recommendations[] = __( 'Enable premium compression with an API key or Easy IO.', 'ewww-image-optimizer-cloud' );
		}
		$disable_level = true;
	}
	...
	if ( get_option( 'easyio_exactdn' ) ) {
		...
		if ( $exactdn->get_exactdn_domain() && $exactdn->verify_domain( $exactdn->get_exactdn_domain() ) ) {
			...
			if ( ewww_image_optimizer_get_option( 'exactdn_lossy' ) ) {
				...
				$compress_recommendations[] = __( 'Enable premium compression.', 'ewww-image-optimizer-cloud' ) . ewwwio_get_help_link( 'https://docs.ewww.io/article/47-getting-more-from-exactdn', '59de6631042863379ddc953c' );
			}
			$exactdn_enabled = true;
		}
	} elseif ( ! ewww_image_optimizer_get_option( 'ewww_image_optimizer_exactdn' ) ) {
		$resize_recommendations[] = __( 'Enable Easy IO for automatic resizing.', 'ewww-image-optimizer-cloud' ) . ewwwio_get_help_link( 'https://docs.ewww.io/article/44-introduction-to-exactdn', '59bc5ad6042863033a1ce370,5c0042892c7d3a31944e88a4' );
	...
	if (
		...
		$resize_recommendations[] = __( 'Configure maximum image dimensions in Resize settings.', 'ewww-image-optimizer-cloud' ) . ewwwio_get_help_link( 'https://docs.ewww.io/article/41-resize-settings', '59849911042863033a1ba5f9' );
	}
	$jpg_quality = apply_filters( 'jpeg_quality', 82, 'image_resize' );
	if ( $jpg_quality < 91 && $jpg_quality > 49 ) {
		$resize_score += 20;
	} else {
		$resize_recommendations[] = __( 'JPG quality level should be between 50 and 90 for optimal resizing.', 'ewww-image-optimizer-cloud' ) . ewwwio_get_help_link( 'https://docs.ewww.io/article/11-advanced-configuration', '58542afac697912ffd6c18c0,58543c69c697912ffd6c19a7' );
	}
	$skip = ewww_image_optimizer_skip_tools();
	if ( ! $skip['jpegtran'] && ! EWWW_IMAGE_OPTIMIZER_NOEXEC ) {
		...
		if ( ! empty( $jpegtran_installed ) ) {
			...
			$compress_recommendations[] = __( 'Install jpegtran.', 'ewww-image-optimizer-cloud' ) . ewwwio_get_help_link( 'https://docs.ewww.io/article/6-the-plugin-says-i-m-missing-something', '585371e3c697912ffd6c0ba1' );
		}
	}
	if ( ! $skip['optipng'] && ! EWWW_IMAGE_OPTIMIZER_NOEXEC ) {
		...
		if ( ! empty( $optipng_version ) ) {
			...
			$compress_recommendations[] = __( 'Install optipng.', 'ewww-image-optimizer-cloud' ) . ewwwio_get_help_link( 'https://docs.ewww.io/article/6-the-plugin-says-i-m-missing-something', '585371e3c697912ffd6c0ba1' );
		}
	}
	if ( ! $skip['pngout'] && ! EWWW_IMAGE_OPTIMIZER_NOEXEC ) {
		...
		if ( ! empty( $pngout_version ) ) {
			...
			$compress_recommendations[] = __( 'Install pngout', 'ewww-image-optimizer-cloud' ) . ': <a href="' . admin_url( 'admin.php?action=ewww_image_optimizer_install_pngout' ) . '">' . esc_html__( 'automatically', 'ewww-image-optimizer-cloud' ) . '</a> | <a href="https://docs.ewww.io/article/13-installing-pngout" data-beacon-article="5854531bc697912ffd6c1afa">' . esc_html__( 'manually', 'ewww-image-optimizer-cloud' ) . '</a>';
		}
	}
	if ( ! $skip['pngquant'] && ! EWWW_IMAGE_OPTIMIZER_NOEXEC ) {
		...
		if ( ! empty( $pngquant_version ) ) {
			...
			$compress_recommendations[] = __( 'Install pngquant.', 'ewww-image-optimizer-cloud' ) . ewwwio_get_help_link( 'https://docs.ewww.io/article/6-the-plugin-says-i-m-missing-something', '585371e3c697912ffd6c0ba1' );
		}
	}
	if ( ! $skip['gifsicle'] && ! EWWW_IMAGE_OPTIMIZER_NOEXEC ) {
		...
		if ( ! empty( $gifsicle_version ) ) {
			...
			$compress_recommendations[] = __( 'Install gifsicle.', 'ewww-image-optimizer-cloud' ) . ewwwio_get_help_link( 'https://docs.ewww.io/article/6-the-plugin-says-i-m-missing-something', '585371e3c697912ffd6c0ba1' );
		}
	}
	if ( EWWW_IMAGE_OPTIMIZER_CWEBP && ! $skip['webp'] && ! EWWW_IMAGE_OPTIMIZER_NOEXEC ) {
		...
		if ( ! empty( $webp_version ) ) {
			...
			$compress_recommendations[] = __( 'Install webp.', 'ewww-image-optimizer-cloud' ) . ewwwio_get_help_link( 'https://docs.ewww.io/article/6-the-plugin-says-i-m-missing-something', '585371e3c697912ffd6c0ba1' );
		}
	}
	if (
		...
		$compress_recommendations[] = __( 'Remove metadata from JPG images.', 'ewww-image-optimizer-cloud' ) . ewwwio_get_help_link( 'https://docs.ewww.io/article/7-basic-configuration', '585373d5c697912ffd6c0bb2' );
	}
	if ( ! ewww_image_optimizer_easy_active() && ! $free_exec ) {
		if ( 0 === (int) ewww_image_optimizer_get_option( 'ewww_image_optimizer_jpg_level' ) ) {
			$compress_recommendations[] = __( 'Enable JPG compression.', 'ewww-image-optimizer-cloud' );
		}
		if ( 0 === (int) ewww_image_optimizer_get_option( 'ewww_image_optimizer_png_level' ) ) {
			$compress_recommendations[] = __( 'Enable PNG compression.', 'ewww-image-optimizer-cloud' );
		}
		if ( 0 === (int) ewww_image_optimizer_get_option( 'ewww_image_optimizer_gif_level' ) ) {
			$compress_recommendations[] = __( 'Enable GIF compression.', 'ewww-image-optimizer-cloud' );
		}
	}
					...
					<div class="ewww-row"><ul class="ewww-blocks">
						<li><div id="ewww-compress" class="ewww-status-detail">
							...
							<div id="ewww-compress-recommend" class="ewww-recommend">
								<strong><?php echo ( (int) $compress_score < 100 ? esc_html__( 'How do I get to 100%?', 'ewww-image-optimizer-cloud' ) : esc_html__( 'You got the perfect score!', 'ewww-image-optimizer-cloud' ) ); ?></strong>
	...
	<?php endif; ?>
							...
							<p><strong><?php esc_html_e( 'Compress', 'ewww-image-optimizer-cloud' ); ?></strong></p>
							<p><?php esc_html_e( 'Reduce the file size of your images without affecting quality.', 'ewww-image-optimizer-cloud' ); ?></p>
						...
						<li><div id="ewww-resize" class="ewww-status-detail">
							...
							<div id="ewww-resize-recommend" class="ewww-recommend">
								<strong><?php echo ( (int) $resize_score < 100 ? esc_html__( 'How do I get to 100%?', 'ewww-image-optimizer-cloud' ) : esc_html__( 'You got the perfect score!', 'ewww-image-optimizer-cloud' ) ); ?></strong>
	...
	<?php endif; ?>
							...
							<p><strong><?php esc_html_e( 'Resize', 'ewww-image-optimizer-cloud' ); ?></strong></p>
							<p><?php esc_html_e( 'Scale or reduce the dimensions of your images for more savings.', 'ewww-image-optimizer-cloud' ); ?></p>
						</div><!-- end .ewww-status-detail --></li>
	<?php
	if ( $total_savings > 0 ) {
