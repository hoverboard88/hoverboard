...
function ewww_image_optimizer_aux_images() {
	...
	if ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_debug' ) ) {
		...
		echo '<p><b>' . esc_html__( 'Debugging Information', 'ewww-image-optimizer-cloud' ) . ':</b> <button id="ewww-copy-debug" class="button button-secondary" type="button">' . esc_html__( 'Copy', 'ewww-image-optimizer-cloud' ) . '</button>';
...
function ewww_image_optimizer_aux_images_table() {
	...
	if ( ! empty( $search ) ) {
		...
		if ( $search_count < $per_page ) {
			...
			$output['search_result'] = sprintf( esc_html__( '%d items found', 'ewww-image-optimizer-cloud' ), count( $already_optimized ) );
		} else {
			/* translators: 1: number of image records displayed, 2: number of total records found */
			$output['search_result'] = sprintf( esc_html__( '%1$d items displayed of %2$d records found', 'ewww-image-optimizer-cloud' ), count( $already_optimized ), $search_count );
		}
		...
		if ( empty( $output['search_result'] ) ) {
			/* translators: %d: number of image records found */
			$output['search_result'] = sprintf( esc_html__( '%d items displayed', 'ewww-image-optimizer-cloud' ), count( $already_optimized ) );
		}
	}
	/* translators: 1: current page in list of images 2: total pages for list of images */
	$output['pagination']   = sprintf( esc_html__( 'page %1$d of %2$d', 'ewww-image-optimizer-cloud' ), (int) $_POST['ewww_offset'] + 1, $total );
	...
	$output['table'] = '<table class="wp-list-table widefat media" cellspacing="0"><thead><tr><th>&nbsp;</th><th>' .
		esc_html__( 'Filename', 'ewww-image-optimizer-cloud' ) . '</th><th>' .
		esc_html__( 'Image Type', 'ewww-image-optimizer-cloud' ) . '</th><th>' .
		esc_html__( 'Last Optimized', 'ewww-image-optimizer-cloud' ) . '</th><th>' .
		esc_html__( 'Image Optimizer', 'ewww-image-optimizer-cloud' ) . '</th></tr></thead>';
	$alternate       = true;
	foreach ( $already_optimized as $optimized_image ) {
		...
		if ( ewww_image_optimizer_stream_wrapped( $file ) ) {
			...
			$type = esc_html__( 'Amazon S3 image', 'ewww-image-optimizer-cloud' );
			// Get a human readable filesize.
			$file_size = ewww_image_optimizer_size_format( $optimized_image['image_size'] );
			/* translators: %s: human-readable filesize */
			$size_string = sprintf( esc_html__( 'Image Size: %s', 'ewww-image-optimizer-cloud' ), $file_size );
			...
			if ( $debug_query ) {
				...
				$output['table'] .= '<br>' . sprintf( esc_html__( 'Number of attempted optimizations: %d', 'ewww-image-optimizer-cloud' ), $optimized_image['updates'] );
				if ( is_array( $trace ) ) {
					$output['table'] .= '<br>' . esc_html__( 'PHP trace:', 'ewww-image-optimizer-cloud' );
					$i                = 0;
					...
					$output['table'] .= '<br>' . esc_html__( 'No PHP trace available, enable Debugging option to store trace logs.', 'ewww-image-optimizer-cloud' );
				}
			}
			$output['table'] .= '</td>';
			...
			$output['table'] .= "<td>$savings<br>$size_string<br>" .
				'<a class="ewww-remove-image" data-id="' . (int) $optimized_image['id'] . '">' . esc_html__( 'Remove from history', 'ewww-image-optimizer-cloud' ) . '</a>' .
				( $optimized_image['backup'] ? '<br><a class="ewww-restore-image" data-id="' . (int) $optimized_image['id'] . '">' . esc_html__( 'Restore original', 'ewww-image-optimizer-cloud' ) . '</a>' : '' ) .
				'</td>';
			$output['table'] .= '</tr>';
			$alternate        = ! $alternate;
			...
			$output['table'] .= '<tr ' . ( $alternate ? "class='alternate' " : '' ) . 'id="ewww-image-' . $optimized_image['id'] . '">';
			$output['table'] .= "<td style='width:50px;' class='column-icon'>" . esc_html__( 'file not found', 'ewww-image-optimizer-cloud' ) . '</td>';
			$output['table'] .= "<td class='title'>...$image_name";
			if ( $debug_query ) {
				/* translators: %d: number of re-optimizations */
				$output['table'] .= '<br>' . sprintf( esc_html__( 'Number of attempted optimizations: %d', 'ewww-image-optimizer-cloud' ), $optimized_image['updates'] );
...
function ewww_image_optimizer_aux_images_clear_all() {
	...
	if ( $wpdb->query( "TRUNCATE $wpdb->ewwwio_images" ) ) {
		die( esc_html__( 'All records have been removed from the optimization history.', 'ewww-image-optimizer-cloud' ) );
	}
	ewwwio_memory( __FUNCTION__ );
	die();
}
	...
	foreach ( $converted_images as $optimized_image ) {
		...
		if ( ! ewww_image_optimizer_stream_wrapped( $file ) && ewwwio_is_file( $file ) ) {
			...
			if ( ewwwio_delete_file( $file ) ) {
				...
				die( wp_json_encode( array( 'error' => sprintf( esc_html__( 'Could not delete %s, please remove manually or fix permissions and try again.', 'ewww-image-optimizer-cloud' ), esc_html( $file ) ) ) ) );
			}
		}
		$wpdb->update(
			$wpdb->ewwwio_images,
	...
	foreach ( $optimized_images as $optimized_image ) {
		...
		if ( ! ewww_image_optimizer_stream_wrapped( $file ) && ewwwio_is_file( $file . '.webp' ) ) {
			...
			if ( ewwwio_delete_file( $file . '.webp' ) ) {
				...
				/* translators: %s: file name */
				die( wp_json_encode( array( 'error' => sprintf( esc_html__( 'Could not delete %s, please remove manually or fix permissions and try again.', 'ewww-image-optimizer-cloud' ), esc_html( $file . '.webp' ) ) ) ) );
			}
		}
...
function ewww_image_optimizer_ajax_delete_original() {
	...
	if ( empty( $_POST['attachment_id'] ) ) {
		die( wp_json_encode( array( 'error' => esc_html__( 'Missing attachment ID number.', 'ewww-image-optimizer-cloud' ) ) ) );
	}
...
function ewww_image_optimizer_get_all_attachments() {
	...
	if ( empty( $attachments ) || ! is_countable( $attachments ) || 0 === count( $attachments ) ) {
		die( wp_json_encode( array( 'error' => esc_html__( 'No media uploads found.', 'ewww-image-optimizer-cloud' ) ) ) );
	}
	...
	foreach ( $iterator as $file ) {
		...
		if (
			...
			die(
				wp_json_encode(
					array(
						'remaining' => '<p>' . esc_html__( 'Stage 2, please wait.', 'ewww-image-optimizer-cloud' ) . "&nbsp;<img src='$loading_image' /></p>",
						'notice'    => '',
					)
				)
			);
		...
		if ( $ewww_scan && 0 === $file_counter % 100 && ! ewwwio_check_memory_available( 2097000 ) ) {
			...
			if ( $file_counter < 100 ) {
				...
				die(
					wp_json_encode(
						array(
							'error' => esc_html__( 'Stage 2 unable to complete due to memory restrictions. Please increase the memory_limit setting for PHP and try again.', 'ewww-image-optimizer-cloud' ),
						)
					)
				);
			}
...
function ewww_image_optimizer_aux_images_script( $hook = '' ) {
	...
	if ( wp_doing_ajax() && 'ewww-image-optimizer-auto' !== $hook && ( ! defined( 'WP_CLI' ) || ! WP_CLI ) ) {
		...
		$ready_msg = sprintf( esc_html( _n( 'There is %s image ready to optimize.', 'There are %s images ready to optimize.', $image_count, 'ewww-image-optimizer-cloud' ) ), '<strong>' . number_format_i18n( $image_count ) . '</strong>' );
		if ( is_array( $usage ) && ! $usage['metered'] ) {
			$credits_available = $usage['licensed'] - $usage['consumed'];
			if ( $credits_available < $image_count ) {
				$ready_msg .= ' ' . esc_html__( 'You do not appear to have enough image credits to complete this operation.', 'ewww-image-optimizer-cloud' );
			}
		}
		if ( $image_count > 1000 ) {
			$ready_msg .= ' <a href="https://docs.ewww.io/article/20-why-do-i-have-so-many-images-on-my-site" target="_blank" data-beacon-article="58598744c697912ffd6c3eb4">' . esc_html__( 'Why are there so many images?', 'ewww-image-optimizer-cloud' ) . '</a>';
		}
		ewwwio_ob_clean();
		die(
			wp_json_encode(
				array(
					...
					'message'      => $ready_msg,
					/* translators: %s: number of images */
					'start_button' => esc_attr( sprintf( __( 'Optimize %s images', 'ewww-image-optimizer-cloud' ), number_format_i18n( $image_count ) ) ),
				)
			)
		);
...
function ewww_image_optimizer_aux_images_cleanup( $auto = false ) {
	...
	if ( ! $auto ) {
		...
		echo '<p><b>' . esc_html__( 'Finished', 'ewww-image-optimizer-cloud' ) . '</b></p>';
		ewwwio_memory( __FUNCTION__ );
		die();
	}
}
