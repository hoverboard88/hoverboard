...
function ewww_image_optimizer_aux_images() {
	...
	if ( ! empty( $db_convert ) ) {
		$output .= '<p class="ewww-bulk-info">' . esc_html__( 'The database schema has changed, you need to convert to the new format.', 'ewww-image-optimizer-cloud' ) . '</p>';
		...
		$output .= '<button id="ewww-table-convert" type="submit" class="button-secondary action">' . esc_html__( 'Convert Table', 'ewww-image-optimizer-cloud' ) . '</button>';
		$output .= '</form>';
	}
	...
	$output .= "<p id='ewww-table-info' class='ewww-bulk-info' $display>" . sprintf( esc_html__( 'The plugin keeps track of already optimized images to prevent re-optimization. There are %d images that have been optimized so far.', 'ewww-image-optimizer-cloud' ), $already_optimized ) . '</p>';
	$output .= "<form id='ewww-show-table' class='ewww-bulk-form' method='post' action='' $display>";
	$output .= '<button type="submit" class="button-secondary action">' . esc_html__( 'Show Optimized Images', 'ewww-image-optimizer-cloud' ) . '</button>';
	$output .= '</form>';
	...
	if ( ewww_image_optimizer_get_option( 'ewww_image_optimizer_debug' ) ) {
		...
		$output .= '<p style="clear:both"><b>' . esc_html__( 'Debugging Information', 'ewww-image-optimizer-cloud' ) . ':</b> <button id="ewww-copy-debug" class="button button-secondary">' . esc_html__( 'Copy', 'ewww-image-optimizer-cloud' ) . '</button>';
		if ( is_file( EWWW_IMAGE_OPTIMIZER_PLUGIN_PATH . 'debug.log' ) ) {
			$debug_log_url = plugins_url( '/debug.log', __FILE__ );
			$output       .= "&emsp;<a href='$debug_log_url'>" . esc_html( 'View Debug Log', 'ewww-image-optimizer-cloud' ) . "</a> - <a href='admin.php?action=ewww_image_optimizer_delete_debug_log'>" . esc_html( 'Remove Debug Log', 'ewww-image-optimizer-cloud' ) . '</a>';
		}
		$output           .= '</p>';
...
function ewww_image_optimizer_aux_images_table() {
	...
	echo '<br /><table class="wp-list-table widefat media" cellspacing="0"><thead><tr><th>&nbsp;</th><th>' . esc_html__( 'Filename', 'ewww-image-optimizer-cloud' ) . '</th><th>' . esc_html__( 'Image Type', 'ewww-image-optimizer-cloud' ) . '</th><th>' . esc_html__( 'Image Optimizer', 'ewww-image-optimizer-cloud' ) . '</th></tr></thead>';
	$alternate = true;
		...
		if ( strpos( $file, 's3' ) === 0 ) {
			...
			id="ewww-image-<?php echo $optimized_image['id']; ?>">
				...
				<td>
					...
					<a class="removeimage" onclick="ewwwRemoveImage( <?php echo $optimized_image['id']; ?> )"><?php esc_html_e( 'Remove from table', 'ewww-image-optimizer-cloud' ); ?></a>
				<?php	if ( $optimized_image['backup'] ) { ?>
					<br><a class="restoreimage" onclick="ewwwRestoreImage( <?php echo $optimized_image['id']; ?> )"><?php esc_html_e( 'Restore original', 'ewww-image-optimizer-cloud' ); ?></a>
				<?php	} ?>
				</td>
			</tr>
			<?php
 ...
 * @global object $wpdb
 */
function ewww_image_optimizer_update_scanned_image( $id, $gallery = 'media' ) {
	global $wpdb;
	$wpdb->update(
		$wpdb->ewwwio_queue,
		array(
 ...
 * @global object $wpdb
 */
function ewww_image_optimizer_delete_queued_image( $id, $gallery = 'media' ) {
	global $wpdb;
	$wpdb->delete(
		$wpdb->ewwwio_queue,
		array(
...
function ewww_image_optimizer_aux_images_script( $hook = '' ) {
	...
	if ( wp_doing_ajax() ) {
		...
		ewwwio_memory( __FUNCTION__ );
		/* translators: %d: number of images */
		$ready_msg = sprintf( esc_html( _n( 'There is %d image ready to optimize.', 'There are %d images ready to optimize.', $image_count, 'ewww-image-optimizer-cloud' ) ), $image_count );
