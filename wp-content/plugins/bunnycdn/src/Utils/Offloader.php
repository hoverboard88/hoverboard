...
class Offloader
{
    ...
    public function __construct(Client $api, AttachmentCounter $attachmentCounter, OffloaderConfig $config)
    {
        $this->api = $api;
    ...
    public function updateStoragePassword(): void
    {
        ...
        $storage = new StorageClient($this->config->getStoragePassword(), $this->config->getStorageZone(), OffloaderConfig::STORAGE_REGION_SSD_MAIN);
        ...
        try {
            ...
            $storage = new StorageClient($storageZone->getPassword(), $storageZone->getName(), OffloaderConfig::STORAGE_REGION_SSD_MAIN);
            $storage->putContents(self::PASSWORD_CHECK_TEST_PATH, '');
            $storage->delete(self::PASSWORD_CHECK_TEST_PATH);
