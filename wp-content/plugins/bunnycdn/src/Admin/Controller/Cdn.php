...
class Cdn implements ControllerInterface
{
    ...
    public function run(bool $isAjax): void
    {
        ...
        if (!empty($_POST)) {
            ...
            if (isset($_POST['perform']) && 'cache-purge' === $_POST['perform']) {
        ...
        if ($config->isAccelerated()) {
            ...
            $this->container->renderTemplateFile('cdn.accelerated.php', [
                'config' => $config,
                'error' => $error,
                'isAccelerated' => $isRequestAccelerated,
        ...
        $this->container->renderTemplateFile('cdn.config.php', [
            'config' => $config,
            'error' => $error,
            'hostnames' => $hostnames,
            'hostnameWarning' => $hostnameWarning,
            'showApiKeyAlert' => $showApiKeyAlert,
            'showCdnAccelerationAlert' => $showCdnAccelerationAlert,
            'showSuccess' => $showSuccess,
            'suggestAcceleration' => $config->isEnabled() && $isRequestAccelerated,
        ], ['cssClass' => 'cdn']);
    }
