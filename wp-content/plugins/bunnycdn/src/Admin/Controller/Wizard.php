...
class Wizard implements ControllerInterface
{
    ...
    public function run(bool $isAjax): void
    {
        ...
        $this->container->renderTemplateFile('wizard.error.php', [
            'error' => 'Invalid wizard step.',
            'wizardUrl' => $wizardUrl,
        ], ['cssClass' => 'wizard error'], '_base.wizard.php');
    }
    ...
    private function step1(): void
    {
        ...
        $this->container->renderTemplateFile('wizard.1.php', [
            'continueUrl' => $continueUrl,
            'agencyModeUrl' => $agencyModeUrl,
        ], ['cssClass' => 'wizard', 'step' => 1], '_base.wizard.php');
    }
    ...
    private function step2(): void
    {
        ...
        $cdnAcceleration = new CdnAcceleration(
            $this->container->getApiClient(),
            $_SERVER,
            $this->container->getAttachmentCounter(),
            'agency' === $mode,
            false,
            false,
            null,
        ...
        if ($cdnAcceleration->isRequestAccelerated()) {
            try {
                ...
                $url = (is_ssl() ? 'https://' : 'http://').$hostname;
            } catch (\Exception $e) {
                $useCdnAcceleration = false;
            }
        }
        ...
        if (!empty($_POST)) {
            ...
            if (!$useCdnAcceleration) {
                try {
                    ...
                    if (-1 === $pullzoneId) {
                        ...
                        $matchingIds = array_map(function (Pullzone\Info $item) {
                            return $item->getId();
                        }, $matchingPullzones);
                    ...
                    $this->container->renderTemplateFile('wizard.2.php', [
                        'formUrl' => $formUrl,
                        'url' => $url,
                        'backUrl' => $backUrl,
                        'mode' => $mode,
                        'error' => $e->getMessage(),
                        'pullzones' => $matchingPullzones,
                        'isAccelerated' => $cdnAcceleration->isRequestAccelerated(),
                    ], ['cssClass' => 'wizard', 'step' => 2], '_base.wizard.php');
            ...
            $cdnConfig = new CdnConfig(
                $cdnAcceleration->isRequestAccelerated() ? CdnConfig::STATUS_ACCELERATED : CdnConfig::STATUS_ENABLED,
                $pullzone->getId(),
                $pullzone->getName(),
                $hostname,
                $url,
                CdnConfig::DEFAULT_VALUES['excluded'],
                CdnConfig::DEFAULT_VALUES['included'],
                CdnConfig::DEFAULT_VALUES['disable_admin'],
                'agency' === $mode,
            );
    ...
    {
        $overviewUrl = add_query_arg([
            'page' => 'bunnycdn',
        ], admin_url('admin.php'));

        $this->container->renderTemplateFile('wizard.3.php', [
            'overviewUrl' => $overviewUrl,
        ], ['cssClass' => 'wizard', 'step' => 3], '_base.wizard.php');
    }
    ...
    private function step2CreatePullzone(string $originUrl): Pullzone\Info
    {
        for ($i = 0; $i < 5; ++$i) {
            try {
                ...
                return $this->container->getApiClient()->createPullzoneForCdn($name, $originUrl);
